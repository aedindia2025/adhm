...
$stmt = $mysqli->prepare("SELECT * FROM std_app_s AS test WHERE test.status = 1 AND test.is_delete = 0  AND NOT EXISTS ( SELECT 1 FROM std_reg_s AS reg WHERE reg.unique_id = test.unique_id ) ORDER BY test.entry_date ASC");
$stmt->execute();
...
if ($result) {
    ...
    foreach ($res_array as $key => $value) {
        ...
        if ($data["count"]) {
            ...
            $stmt = $mysqli->prepare("SELECT sanctioned_strength, transfer_strength, additional_strength  FROM hostel_name WHERE unique_id = ?");
            $stmt->bind_param("s", $value['hostel_1']);
            $stmt->execute();
            $hostel_obj = $stmt->get_result();
            $stmt->close();

            if ($hostel_obj && $hostel_data = $hostel_obj->fetch_assoc()) {
                $sanctioned_strength = $hostel_data['sanctioned_strength'];
                $additional_strength = $hostel_data['additional_strength'];
                $transfer_count = $hostel_data['transfer_strength'];
                
                // Calculate the available strength
                $strength = $sanctioned_strength + $additional_strength - $transfer_count;
                // $strength = $one_count - $transfer_count;

                // Fetch the actual count of students in the same hostel
                $stmt = $mysqli->prepare("SELECT COUNT(id) AS actual_count FROM std_reg_s WHERE hostel_1 = ?");
                $stmt->bind_param("s", $value['hostel_1']);
                $stmt->execute();
                $actual_count_obj = $stmt->get_result();
                $stmt->close();

                if ($actual_count_obj && $actual_count_data = $actual_count_obj->fetch_assoc()) {
                    $actual_count = $actual_count_data['actual_count'];

                    // If strength < actual_count, skip this student
                    if ($strength < $actual_count) {
                        echo "Skipping student with unique_id: " . $value['unique_id'] . " (strength < actual_count)\n";
                        continue; // Skip this iteration and move to the next student
                    }
                }
            }

            $value['s1_unique_id'] = $value['unique_id'];
