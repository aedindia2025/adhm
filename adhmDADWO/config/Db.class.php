...
class Db
{
	 ...
	 * @param null   $prefix
	 */
	public function __construct($driver = 'mysql', $host = 'localhost', $user = 'root', $pass = '4/rb5sO2s3TpL4gu', $name = 'adi_dravidar', $charset = 'utf8', $prefix = null)
	{
	 ...
	 */
	public function select($table, $where = [], $limit = null, $start = null, $order_by = [], $group_by = [])
	{
		...
		$sql_str   .= ' GROUP BY screen_unique_id ';
		...
		if (!empty($group_by))
		{
			...
			// $add_comma = false;
			// foreach ($group_by as $column => $group)
			// {
			// 	if ($add_comma)
			// 	{
			// 		$sql_str .= ', ';
