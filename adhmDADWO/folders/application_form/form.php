...
$student_status_options = select_option($student_status_options);
	...
	<div class="content">
		...
		<div class="container-fluid">
			<div class="container">
								...
								<ul class="nav nav-pills bg-nav-pills nav-justified mb-3 mt-3">
									<li class="nav-item">
										<a href="#home1" data-bs-toggle="tab" aria-expanded="false"
											...
											<span class="d-none d-md-block">விடுதி விண்ணப்பம் /
												Hostel Application</span>
										</a>
									</li>
									<li class="nav-item">
										<a href="#profile1" data-bs-toggle="tab" aria-expanded="true"
											...
											<span class="d-none d-md-block">விண்ணப்ப திருத்தம் /
												Application Edit</span>
										</a>
									</li>
								...
								<div class="tab-content">
									<div class="tab-pane  show active" id="home1">
												...
												<div class="row student-cat">
													...
													<div class="col-md-6">
														<select class="form-select" id="student_status"
															onchange="get_aad_field()">
															<?php echo $student_status_options; ?>
														</select>
													</div>
												</div>
											</div>
											<div id="formotpfields" style="display:none">
												
													<div class="row mt-3">
														<div class="col-md-3">
															<label class="form-label" for="userName2">கல்வி ஆண்டு /
																Academic
																Year</label>
														</div>
														<div class="col-md-3">
														...
														<div class="col-md-3">
															<label class="form-label" for="userName2">நீங்கள்
																எங்கே படிக்கீறீர்கள் ? / Where are
																you
																studying?</label>
														</div>
														<div class="col-md-3">
														...
														<div class="col-md-3" id="aad_field">
															...
															<input type="text" id="aadhar_no" class="form-control"
																...
																onkeyup="onAadharKeyPress()">
															<span id="aadhaarError" class="error"
										...
										<!-- <div class="col-md-12"> -->
															<span id="admissionMessage"
																style="text-align: left;color: red; display: none;font-weight: 700;font-size: 13px;">You
																can proceed with
																the application only after you've received admission to
																the
																college.</span>
										<!-- </div> -->
									...
									<div class="tab-pane" id="profile1">
										<div class="row spel-app normal-imput">
											<div class="col-md-3">
												<label class="form-label" for="userName2">மாணாக்கரின்
													ஆதார் எண்ணினை பதிவிடவும் / Enter the
													student Aadhaar Number</label>

												<input type="hidden" class="form-control" id="unique_id"
													name="unique_id">
											</div>
											<div class="col-md-3">
												<input type="text" id="edit_aadhar_no" class="form-control"
													maxlength="12" oninput="valid_aadhar_number(this)"
													onkeyup="onAadharEditKeyPress()">
										...
										<div class="row">
											<div class="col-md-12 mt-3 align-self-center text-center">
												<input type="button" class="comm-btn" id="edit_gen_otp"
													onclick="edit_gen_otp();" value="Generate OTP">
											</div>
										</div>
									</div>
								</div>
						...
						<div class="modal-content">
							<div class="modal-body p-4 out-mar">
								<div class="text-center model-design">
									<div id="fresh_app" class="position-absolute top-0 end-0 me-3 mt-3"></div>
									<center><img src="../img/otp.webp"></center>
									...
									<p class="mt-0">We have sent a verification code to your Aadhaar Linked Mobile
										Number</p><br>
									<div id="inputs">
				...
				<div id="edit-login-modal" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true"
					data-backdrop="static" data-bs-backdrop="static" data-bs-keyboard="false">
					<div class="modal-dialog modal-md">
						<div class="modal-content">
							<div class="modal-body p-4 out-mar">
...
<script>
	...
	function get_aad_field() {


		var student_status = $("#student_status").val();

		if (student_status == 1) {
			$("#formotpfields").css("display", "inline-flex");
			$("#gen_otp").css("display", "inline-flex");
			$("#aad_lbl").hide();
			$("#aad_field").hide();
			$("#gen_otp").val('Submit');
			...
			$("#formotpfields").css("display", "inline-flex");
			$("#gen_otp").css("display", "inline-flex");
			$("#gen_otp").val('Generate OTP');

			$("#aad_lbl").show();
			$("#aad_field").show();
		} else {
			$("#formotpfields").css("display", "none");
			$("#gen_otp").css("display", "none");
			$("#gen_otp").val('');
	...
	function gen_otp() {
		...
		if (student_status == 1) {
			...
			if (hostel_type != '' && app_type != '') {

				var ajax_url = "folders/application_form/crud.php";
				var data = "&hostel_type=" + base64Encode(hostel_type) + "&academic_year=" + base64Encode(academic_year) + "&app_type=" + base64Encode(app_type) + "&std_name=" + base64Encode(name) + "&action=createupdate&student_status=" + base64Encode(student_status);
				...
				$.ajax({
					...
					success: function (data) {
						...
						var encrypted_unique_id = obj.encrypted_unique_id;
						var url = obj.url;


						if (msg == 'otp') {

							form_otp_sweetalert("create_app", "index.php?file=" + url + "&unique_id=" + encrypted_unique_id);
						} else {
							form_otp_sweetalert(msg);
				...
				form_otp_sweetalert("form_alert");
			}
		}
		else {
			...
			if (aadhar_no != '' && hostel_type != '' && app_type != '') {
				...
				var ajax_url = "folders/application_form/crud_v1.php";
				var data = {
				...
				$.ajax({
							...
							$.ajax({
								...
								success: function (status) {
									...
									if (uuid_msg == "not_found") {
										...
										$.ajax({
											...
											success: function (response) {
												...
												if (response.status) {
													...
													if (err != "000") {
														form_otp_sweetalert("aadhar_maintanance", '', '', errdesc);
														$("#gen_otp").prop("disabled", false);
													} else {
										...
										form_otp_sweetalert("aadhar_already");
										$("#gen_otp").prop("disabled", false);
									}
					...
					error: function (xhr, status, error) {
						hideLoader();
						form_otp_sweetalert("failed_conn_auth");
						$("#gen_otp").prop("disabled", false);
						// Handle AJAX error
						console.error(xhr.responseText);
					}
			...
			$.ajax({
				...
				success: function (response) {
					...
					if (response.data.err != "000") {
						...
						form_otp_sweetalert("aadhar_maintanance", '', '', response.data.errdesc);
						...
						if (response.data.errdesc != "OTP validation failed") {
							...
							var dob = poiNode?.getAttribute("dob") || ""; // Use optional chaining
							...
							var addressParts = [co, house, street, po, vtc, dist, state, pc].filter(function (value) {
								return value.trim() !== ""; // Exclude empty values
							});
							...
							// Extract father's name from 'co' if it exists
							var fatherName = co ? co.replace(/^(S\/O|D\/O) ?:? ?/i, "").trim() : "";
							...
							var data1 = "&hostel_type=" + base64Encode(hostel_type) + "&academic_year=" + base64Encode(academic_year) + "&app_type=" + base64Encode(app_type) + "&uuid=" + base64Encode(uuid) + "&std_name=" + base64Encode(name) + "&action=createupdate&student_status=" + base64Encode(student_status);

							$.ajax({
								type: "POST",
								url: "folders/application_form/crud.php",
								data: data1,
								...
								success: function (data) {
									...
									if (msg == 'otp') {
										...
										var a_data = {
											...
											"a_no": aadhar_no,
											"pro_image": pro_image,
											"aadhar_no": base64Encode(aadhar_no),
										...
										form_otp_sweetalert("verify", "index.php?file=" + url + "&unique_id=" + encrypted_unique_id);
...
if (batch_no === '' || batch_no === null) {
									...
									$.ajax({
										...
										form_otp_sweetalert("batch_created");
									}

								} else {
									hideLoader();
									form_otp_sweetalert("no_aadhar");
									$("#edit_gen_otp").prop("disabled", false);
								}
							...
							$.ajax({
								...
								success: function (status) {
									if (status.msg === "uid") {
										...
										form_otp_sweetalert("verify", "index.php?file=" + url + "&unique_id=" + unique_id);

									} else {
										alert("no_uid");
										form_otp_sweetalert("no_uid");
									}
								},
							});
						} else {
	...
	function form_otp_sweetalert(msg = '', url = '', callback = '', title = '') {
...
<script>
	...
	function edit_startTimer() {
		...
		document.getElementById("fresh_app").innerHTML = `
