...
switch ($action) {
    ...
    case 'manual_datatable':
        ...
        $columns = [
            ...
            "father_name",
            "std_degree",
            "(select hostel_id from hostel_name where renewal.hostel_id = hostel_name.unique_id) as hostel_main_id",
            "(select hostel_name from hostel_name where renewal.hostel_id = hostel_name.unique_id) as hostel_name",
            "'' as action",
        ...
        $where = "renewal_opt = ? and hostel_district = ? and manual_submit_status = ?";
        $params = [];
        $renewal_opt = 2;
        $manual_submit_status = 1;
        ...
        if ($limit != "") {
            $stmt->bind_param('isiii', $renewal_opt, $district_id, $manual_submit_status, $start, $limit);
        } else {
            $stmt->bind_param('isi', $renewal_opt, $district_id, $manual_submit_status);
        }
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
               ...
               if ($score == 1) {
                ...
                $row['s_no'] = ++$s_no;
                if ($row['manual_acc_status'] == 0) {
                   $acceptbtn = '<button class="acceptbtn" style="background-color: green; color: white; padding: 5px 10px; border: none; border-radius: 4px; margin-right: 8px;" data-s1_unique_id="' . $row['s1_unique_id'] . '" data-hostel_name="' . $row['hostel_name'] . '" data-std_name="' . $row['std_name'] . '" data-name_diff="' . $name_diff . '" data-inst_distance_check="' . $inst_distance_check . '">Accept</button>';
                    $rejectbtn = '<button class="rejectbtn" style="background-color: red; color: white; padding: 5px 10px; border: none; border-radius: 4px; margin-right: 8px;" data-s1_unique_id="' . $row['s1_unique_id'] . '" data-hostel_name="' . $row['hostel_name'] . '" data-std_name="' . $row['std_name'] . '" data-name_diff="' . $name_diff . '" data-inst_distance_check="' . $inst_distance_check . '">Reject</button>';
                    $row['action'] = $acceptbtn . ' ' . $rejectbtn;
                } elseif ($row['manual_acc_status'] == 1) {
                    $row['action'] = "<p>Accepted</p>";
                } elseif ($row['manual_acc_status'] == 2) {
                    $row['action'] = "<p>Rejected</p>";
                }
    ...
    case 'reject':
        ...
        $update_columns = ["manual_acc_status" => 2, "reject_reason" => $exp_reason];
        $update_where = 's1_unique_id = "' . $s1_unique_id . '"';
        $update_action = $pdo->update("renewal", $update_columns, $update_where);


        echo json_encode(["status" => true, "msg" => "rejected"]);
    ...
    case 'autoRenewal':
        ...
        $update_columns = ["manual_acc_status" => 1, "accept_reason" => $exp_reason];
        $update_where = 's1_unique_id = "' . $s1_unique_id . '"';
        $update_action = $pdo->update("renewal", $update_columns, $update_where);
    ...
    case 'get_hostel_id':
        ...
        $hostel_name_options = hostel_name('', $taluk_id);
