...
switch ($action) {
    case 'createupdate':
        ...
        case 'datatable':
            ...
            $table_details = $table;
            $where = "is_delete = ? AND bill_no IS NOT NULL AND hostel_district = ?";
            $is_delete = "0";
            $bind_params = "is";
            $bind_values = [$is_delete,$_SESSION["district_id"]];
            ...
            if ($result) {
		     ...
		     if ($row['rec_status'] == 1) {
                    ...
                    } elseif ($row['print_status'] == '2') {
                        $button = '<button onclick="rec_status(\'' . $row['bill_no'] . '\')" style="background-color: #3bb6e9; border: none; color: white; padding: 5px 10px; text-align: center; text-decoration: none; display: inline-block; font-size: 12px; border-radius: 4px; cursor: pointer;" >Received</button>';
                        $row['rec_status'] = $button;
                    } else {
...
$unique_id = $row['screen_unique_id'];
                    ...
                    switch ($row['status']) {
                        case 0:
                            $acceptButton = '<button class="accept-btn" data-batch-no="' . $batch_no . '" data-unique-id="' . $row['st_unique_id'] . '">Approve</button>';
                            $rejectButton = '<button class="reject-btn" data-batch-no="' . $batch_no . '" data-unique-id="' . $row['st_unique_id'] . '">Reject</button>';
