...
function batch_no($academic_year="")
    {
        ...
        $stmt_acc = $conn->query("SELECT acc_year FROM academic_year_creation where is_delete = '0' order by s_no desc Limit 1");
        
        $value = $stmt_acc->fetch();

        //  $acmc_year = academic_year($academic_year)[0]['acc_year'];
        $a = str_split($value['acc_year']);
         $splt_acc_yr = $a[0].$a[1].$a[2].$a[3];
        //  $hostel_id = $_SESSION['hostel_id']; 
    ...
    $splt_hos_id = substr($hostel_main_id, 0, 4)  . substr($hostel_main_id, -2);
        ...
        $stmt = $conn->query("SELECT bill_no  FROM bill_submission where is_delete = '0' order by id desc limit 1");
    // echo "SELECT max(batch_no) as batch_no FROM batch_creation where is_delete = '0' and hostel_name = '".$hostel_id."' order by id desc";
        ...
        if ($booking_no == ''){
            ...
            $booking_nos = $splt_acc_yr.'BNO-'.'0001';
        }
        
        else {
            $booking_no += 1;
            
        $booking_nos = $splt_acc_yr.'BNO-'.str_pad($booking_no, 4, '0', STR_PAD_LEFT);
        }
...
function batch_create() {
    ...
    var data = "unique_id=" + unique_id + "&net_total_amount=" + net_total_amount + "&hostel_name=" + hostel_name + "&hostel_taluk=" + hostel_taluk + "&hostel_district=" + hostel_district + "&supplier_name=" + supplier_name + "&file_name=" + file_name+ "&academic_year=" + academic_year+ "&batch_no=" + batch_no;
