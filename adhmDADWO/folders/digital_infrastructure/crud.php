...
switch ($action) {
    ...
    case 'createupdate':
        ...
        $reg_of_land = $_POST["reg_of_land"];
        $area_of_land = $_POST["area_of_land"];
        $con_area_land = $_POST["con_area_land"];
    ...
    case 'datatable':
        ...
        if ($stmt) {
            $stmt->bind_param('iisii', $start, $is_delete, $_SESSION["district_id"], $start, $limit);

            // Execute statement
            $stmt->execute();


            $stmt->bind_result($s_no, $amc_year, $land_type, $owner_of_land, $existing_demolished, $unique_id);
            ...
            while ($stmt->fetch()) {
                ...
                $row = [
                    's_no' => $s_no,
                    'amc_year' => $amc_year,
                    'land_type' => $land_type == "own_land" ? "Having own land" : "Not having own land",
                    'owner_of_land' => $owner_of_land,
                    'existing_demolished' => disname($existing_demolished),
                    'unique_id' => '<a class="btn btn-action specl2-icon" href="javascript:assetInfra_print(\'' . $unique_id . '\')"><button type="button"><i class="fa fa-eye"></i></button></a>'
                ];
    ...
    case 'digital_infra_details_delete':
        ...
        $action_obj = $pdo->update($table_facility_details, $columns, $update_where);
    ...
    case 'buildings_sub_delete':
        ...
        $action_obj = $pdo->update($table_sub_building, $columns, $update_where);
