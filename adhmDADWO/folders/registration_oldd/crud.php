                        ...
                        if ($action_obj_p1) {
                            ...
                            if (!empty($data_p1)) {
                                foreach ($data_p1 as $row_p1) {
                                    ...
                                    if (in_array($row_p1['renewal_status'], [1, 2, 3])) {
                                        ...
                                        if ($renewal_result && $renewal_row = $renewal_result->fetch_assoc()) {
                                            ...
                                            if ($transfer_status != 1) {
                                                $row_p1['sync_status'] = $renewal_row['sync_status'];
...
//         $new_seq_no = 1;
//     } else {
//         $last_seq_no = intval(substr($last_reg_no, -6)); // Extract last 6 digits
//         $new_seq_no = $last_seq_no + 1;
//     }
...
function reg_no($academic_year, $s1_unique_id)
{
    ...
    try {
        ...
        // Get latest academic year from DB
        $sql = $conn->query("SELECT * FROM academic_year_creation WHERE is_delete = '0' ORDER BY s_no DESC LIMIT 1");
        $row = $sql->fetch();
        $acc_year = $row['amc_year'];
        $latest_academic_year = $row['unique_id'];  // Don't overwrite argument
        $splt_acc_yr = substr($acc_year, 0, 4);

        // Use passed academic_year or override if needed
        ...
        SELECT std_reg_no 
        FROM std_reg_s 
        WHERE std_reg_no LIKE :prefix AND academic_year = :academic_year
        ORDER BY CAST(RIGHT(std_reg_no, 6) AS UNSIGNED) DESC 
        LIMIT 1 
        FOR UPDATE
    ");
        $stmt->execute([
            ':prefix' => $splt_acc_yr . 'ADTW%',
            ':academic_year' => $academic_year
        ]);
        $last_reg_no = $stmt->fetchColumn();
        ...
        switch ($hosteltype) {
            case '65f00a259436412348':
                $hosteltype = 'S';
                break;
            case '65f00a327c08582160':
                $hosteltype = 'I';
                break;
            case '65f00a3e3c9a337012':
                $hosteltype = 'D';
                break;
            case '65f00a495599589293':
            case '65f00a53eef3015995':
                $hosteltype = 'C';
                break;
            default:
                $hosteltype = 'X'; // fallback
        }
        ...
        // Insert into register_no or std_reg_s — confirm table name
        $insertStmt = $conn->prepare("INSERT INTO register_no (academic_year, reg_no) VALUES (?, ?)");
        $insertStmt->execute([$academic_year, $reg_no]);
        ...
        if ($e->getCode() == 23000) {
            return reg_no($academic_year, $s1_unique_id);
        }

        // Handle other errors
        error_log("Reg No Generation Error: " . $e->getMessage());
        return false;
    }
}
