...
// 								text: 'Student has been transferred.',
// 								timer: 1000,
// 							});
// 							setTimeout(() => {
...
function transferStudent(btn) {
	...
	// Step 1: Check hostel capacity before showing transfer Swal
	$.ajax({
		url: ajax_url,
		method: "POST",
		data: {
			action: "check_hostel_capacity",
			hostel_unique_id: hostelUnId
		},
		success: function (response) {
			try {
				...
				if (!res.status) {
					Swal.fire({
						icon: 'error',
						title: 'Error',
						text: res.msg || 'Unable to check capacity.'
					});
					return;
				}

				if (res.registered_count >= res.sanctioned_strength) {
					Swal.fire({
						icon: 'warning',
						title: 'Sanctioned Count Exceeded',
						text: "Can't proceed transfer to this hostel.",
					});
					return;
				}

				// Step 2: Continue with transfer confirmation
				Swal.fire({
					title: 'Are you sure?',
					text: `Do you want to transfer this student to Hostel ID: ${hostelId}?`,
					icon: 'question',
				...
				Swal.fire({
					icon: 'error',
					title: 'Error',
					text: 'Invalid response while checking capacity.'
				});
			}
		},
		error: function () {
			Swal.fire({
				icon: 'error',
				title: 'Error',
				text: 'Unable to check hostel capacity. Try again later.'
			});
		}
	});
}
