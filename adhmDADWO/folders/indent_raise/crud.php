...
switch ($action) {
    ...
    case 'item_details':
        ...
        $where = "hostel_id = ? AND is_delete = 0";
        $order_by = "";
        ...
        if ($result) {
            ...
            foreach ($res_array as $key => $value) {
                ...
                // $value['price'] = ($value['quantity'] * $value['unit_price']);

                // Safely convert string inputs to float values before calculation
                $quantity = floatval(str_replace(',', '', $value['quantity'] ?? 0));
                $unit_price = floatval(str_replace(',', '', $value['unit_price'] ?? 0));

                // Calculate price
                $value['price'] = round($quantity * $unit_price, 2);

                // $value['item'] = disname(item_stock_inward($value['item'])[0]['item']) . '  (' . $value['unit'] . ')';

                if (isset($value['item']) && $value['item'] == "C-V") {
                    // Always show unit as Kg for C-V
                    $value['item'] = 'Vegetables (Kg)';
                } else {
                    // Normal case: show actual item name with its unit
                    $itemName = disname(item_stock_inward($value['item'])[0]['item']);
                    $value['item'] = $itemName . ' (' . $value['unit'] . ')';
                }
