...
switch ($action) {
    ...
    case 'sublist_cu':
        ...
        $check_exist = "SELECT COUNT(*) FROM district_percentage_sub WHERE district = ? AND hostel_type = ? AND month = ? AND is_delete = 0 AND unique_id != ?";
        ...
        if ($total > 0) {
            ...
            if ($sublist_unique_id) {
                ...
                SET district = ?, hostel_type = ?, month = ?, percentage = ? WHERE unique_id = ? AND is_delete = 0";

                $stmt = $mysqli->prepare($sql);
                $stmt->bind_param("sssss", $district, $hostel_type, $month, $percentage, $sublist_unique_id);
                ...
                $sql = "INSERT INTO district_percentage_sub (unique_id, screen_unique_id, district, hostel_type, month, percentage) 
                VALUES (?, ?, ?, ?, ?, ?)";

                $stmt = $mysqli->prepare($sql);
                $stmt->bind_param("ssssss", $new_id, $screen_unique_id, $district, $hostel_type, $month, $percentage);
