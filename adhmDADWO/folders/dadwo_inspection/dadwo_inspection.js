...
function leave_print(unique_id = "") {
	...
	var external_window = window.open('folders/dadwo_inspection/print.php?unique_id=' + unique_id, 'onmouseover', 'height=600,width=900,scrollbars=yes,resizable=no,left=200,top=150,toolbar=no,location=no,directories=no,status=no,menubar=no');
...
function dadwo_inspection_cu(unique_id = "") {
	...
	if(inspection_date != '' && taluk_name != '' && hostel_name != '' && description != '' && (image_s != '' || pic != '')){
	// if (image_s != '' || pic != '') {
...
function print(file_name) {
	...
	var excelUrl = "../uploads/inspection/" + file_name;
	var link = document.createElement("a");
	link.href = excelUrl;
	link.download = file_name;
	document.body.appendChild(link);
...
function dadwo_inspection_delete(unique_id = "") {
