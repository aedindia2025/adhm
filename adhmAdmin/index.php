...
<script>
    ...
    // Get the user login status from PHP
...
const resetLogoutTimer = () => {
  ...
  // Set a new logout timer for 15 minutes
  logoutTimer = setTimeout(() => {
    // Redirect to logout page after 15 minutes of inactivity
    window.location.href = 'logout.php';
  }, 900000); // 15 minutes in milliseconds
};

// Reset the logout timer on user interactions
document.addEventListener('mousemove', resetLogoutTimer);
document.addEventListener('keypress', resetLogoutTimer);
document.addEventListener('click', resetLogoutTimer);

// Call the function initially to set the timer when the page loads
resetLogoutTimer();
</script>
      <!-- Main Content -->
 </html>