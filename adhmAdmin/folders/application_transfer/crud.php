...
switch ($action) {
    case 'datatable':
        ...
        $where = "(s.is_delete = 0 AND s.status = 2 AND s.ap_transfer_status = 0)";
    ...
    case 'transfer_datatable':
        ...
        // WHERE clause
        $where = "is_delete = 0 AND is_active = 1 AND district_name IN ($idList) AND gender_type = '$gender'";
    ...
    case 'application_transfer':
        ...
        unset($row['id'], $row['batch_no'], $row['batch_cr_date'], $row['status_upd_date'], $row['status'], $row['submit_status'], $row['created'], $row['updated']);
        ...
        $update_original_std_app_s = $pdo->update("std_app_s", ["ap_transfer_status" => 1], 'unique_id = "' . $s1_unique_id . '"');
