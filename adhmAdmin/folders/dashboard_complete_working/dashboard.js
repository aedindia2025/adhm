        ...
        success: function (responseData) {
            ...
            var productOptions = {
                ...
                yaxis: {
                    tickAmount: 5,
                    min: 0,
                    // max: Math.max(...projectionData) || 5000,
                    max: 500,
                    labels: {
...
function get_total_staffs() {
    ...
    $.ajax({
        ...
        success: function (data) {
            var obj = JSON.parse(data);
            var warden_cnt = obj.warden_cnt;
            var warden_inc_cnt = obj.warden_inc_cnt;
            var cook_cnt = obj.cook_cnt;
            var cook_dep_cnt = obj.cook_dep_cnt;
            var watchman_cnt = obj.watchman_cnt;
            var watchman_dep_cnt = obj.watchman_dep_cnt;
            var sweeper_cnt = obj.sweeper_cnt;
            var sweeper_dep_cnt = obj.sweeper_dep_cnt;
