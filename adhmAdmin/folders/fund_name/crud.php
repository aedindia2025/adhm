...
switch ($action) {
    case 'createupdate':
        ...
        $fund_name = sanitizeInput($_POST['fund_name']);
        $is_active = sanitizeInput($_POST['is_active']);
        $unique_id = sanitizeInput($_POST['unique_id']);

        // Check if the entry already exists
        $select_sql = "SELECT COUNT(unique_id) AS count FROM $table WHERE fund_name = ? AND is_delete = ?";
        if ($unique_id) {
        ...
        if ($unique_id) {
            $stmt->bind_param('sss', $fund_name, $is_delete, $unique_id);
        } else {
            $stmt->bind_param('ss', $fund_name, $is_delete);
        }
        ...
        if ($data['count'] > 0) {
            ...
            if ($unique_id) {
                $update_sql = "UPDATE $table SET fund_name = ?, is_active = ? WHERE unique_id = ?";
                $stmt = $mysqli->prepare($update_sql);
                $stmt->bind_param('sss', $fund_name, $is_active, $unique_id);
            } else {
                $columns = [
                    'fund_name' => $fund_name,
                    'is_active' => $is_active,
                ...
                $insert_sql = "INSERT INTO $table (fund_name, is_active, unique_id) VALUES (?, ?, ?)";
                $stmt = $mysqli->prepare($insert_sql);
                $stmt->bind_param('sss', $fund_name, $is_active, $columns['unique_id']);
            }
