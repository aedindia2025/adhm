...
switch ($action) {
    case 'createupdate':
        ...
        $special_tahsildar = sanitizeInput($_POST["special_tahsildar"]);
        $is_active = sanitizeInput($_POST["is_active"]);
        $unique_id = sanitizeInput($_POST["unique_id"]);
    
    
        // Check if the entry already exists
        $select_sql = "SELECT COUNT(unique_id) AS count FROM $table WHERE special_tahsildar = ? AND is_delete = 0";
        if ($unique_id) {
        ...
        if ($unique_id) {
            $stmt->bind_param("ss", $special_tahsildar, $unique_id);
        } else {
            $stmt->bind_param("s", $special_tahsildar);
        }
	    ...
	    $status = true;
            ...
            if ($unique_id) {
                $update_sql = "UPDATE $table SET district_name = ?, special_tahsildar = ?, is_active = ? WHERE unique_id = ?";
                $stmt = $mysqli->prepare($update_sql);
                $stmt->bind_param("ssss", $district_name, $special_tahsildar, $is_active, $unique_id);
            } else {
                $columns = [
                ...
                $insert_sql = "INSERT INTO $table (district_name, special_tahsildar, is_active, unique_id) VALUES (?, ?, ?, ?)";
                $stmt = $mysqli->prepare($insert_sql);
                $stmt->bind_param("ssss", $district_name, $special_tahsildar, $is_active, $columns['unique_id']);
            }
    ...
    case 'datatable':
        ...
        if ($result) {
            ...
            foreach ($res_array as $key => $value) {
                ...
                $value['special_tahsildar'] = disname($value['special_tahsildar']);
