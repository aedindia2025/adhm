...
switch ($action) {
    case 'createupdate':
        ...
        $sql = 'SELECT COUNT(unique_id) AS count FROM ' . $table . ' WHERE asset_name = ? AND is_delete = 0';
        if ($unique_id) {
            $sql .= ' AND unique_id != ?';
        }
        ...
        if ($unique_id) {
            $stmt->bind_param("ss", $asset_name, $unique_id);
        } else {
	    ...
	    $status = true;
            ...
            if ($unique_id) {
                // Perform update
                $sql = 'UPDATE ' . $table . ' SET asset_category = ?, asset_name = ?, description = ?, is_active = ? WHERE unique_id = ?';
                $stmt = $mysqli->prepare($sql);
                $stmt->bind_param("sssss", $asset_category, $asset_name, $description, $is_active, $unique_id);
            } else {
                // Perform insert
                $sql = 'INSERT INTO ' . $table . ' (asset_category, asset_name, description, is_active, unique_id) VALUES (?, ?, ?, ?, ?)';
                $stmt = $mysqli->prepare($sql);
                $stmt->bind_param("sssss", $asset_category, $asset_name, $description, $is_active, $columns["unique_id"]);
            }
        ...
        $json_array = [
            ...
            "error" => $stmt->error,
            "msg" => $msg,
            // "sql" => $stmt->sqlstate // Uncomment for debugging SQL state
        ];
    ...
    case 'datatable':
        ...
        $columns = [
            "@a:=@a+1 s_no",
            "(SELECT asset_category FROM asset_category WHERE asset_category.unique_id = asset_name_creation.asset_category) AS asset_category",
            "asset_name",
            "description",
            "is_active",
            "unique_id"
        ...
        // Construct table details and additional components
...
$is_delete = "0";
        ...
        if (!empty($limit)) {
            $stmt->bind_param("iiii", $start, $is_delete, $start, $limit);
        }
        ...
        if ($result) {
            ...
            foreach ($res_array as $key => $value) {
                // Perform any data transformations here as needed
                $value['asset_category'] = disname($value['asset_category']);
            ...
            $json_array = [
                ...
                "data" => $data,
                // You may remove "testing" in production
