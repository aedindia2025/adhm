...
var form_name 		= 'Maintenance';
var form_header		= '';
var form_footer 	= '';
var table_name 		= '';
var table_id 		= 'establishment_report_datatable';
	...
	var datatable = table.DataTable({
        ...
        "columnDefs": [
            ...
            { 
			 ...
			 } // Align the fourth, fifth, and sixth columns to the right
        ],
		"ajax"		: {
            destroy : true,
            url 	: ajax_url,
...
//     } 
    ...
    function view_app(unique_id="",type="") {
	
	
		var external_window = window.open('folders/establishment_report/print.php?unique_id='+unique_id+'&type='+type, 'onmouseover', 'height=600,width=900,scrollbars=yes,resizable=no,left=200,top=150,toolbar=no,location=no,directories=no,status=no,menubar=no');
...
$("#export_establishment").click(function () {
	
	// Show the loader
	showLoader();
	$("#export_establishment").prop("disabled", true);
var taluk_name = $("#taluk_name").val();
var hostel_name = $("#hostel_name").val();
// var
	// Use AJAX to request the file
	$.ajax({
		url: "folders/establishment_report/excel.php",
		type: "GET",
        data: {
            taluk_name: taluk_name,
            hostel_name: hostel_name
		...
		success: function (data) {
			...
			link.href = url;
			link.download = 'Establishment_Report.xls'; // Set the filename
			document.body.appendChild(link);
			link.click();
			...
			// Hide the loader
			hideLoader();
			$("#export_establishment").prop("disabled", false);
