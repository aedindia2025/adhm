...
$excel->getActiveSheet()
    ...
    ->setCellValue('AE3', 'Latitude')
    ->setCellValue('AF3', 'Longitude');
...
foreach ($users as $row) {
    ...
    if($row['special_tahsildar']){
        $row['special_tahsildar'] = special_tahsildar($row['special_tahsildar'])[0]['special_tahsildar'];
        }
    ...
    $excel->getActiveSheet()
        ...
        ->setCellValue('AE' . $row_val, $row['latitude'] ?: '-')
        ->setCellValue('AF' . $row_val, $row['longitude'] ?: '-');
...
$excel->getActiveSheet()->getStyle('A3:AF3')->applyFromArray($styleArray);
