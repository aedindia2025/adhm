...
switch ($action) {
    case 'createupdate':
        ...
        $select_sql = "SELECT COUNT(unique_id) AS count FROM $table WHERE taluk_name = ? AND is_delete = 0";
        if ($unique_id) {
        ...
        if ($unique_id) {
            $stmt->bind_param("ss", $taluk_name, $unique_id);
        } else {
	    ...
	    $status = true;
            ...
            if ($unique_id) {
                $update_sql = "UPDATE $table SET district_name = ?, taluk_name = ?, is_active = ? WHERE unique_id = ?";
                $stmt = $mysqli->prepare($update_sql);
                $stmt->bind_param("ssss", $district_name, $taluk_name, $is_active, $unique_id);
            } else {
                $columns = [
                ...
                $insert_sql = "INSERT INTO $table (district_name, taluk_name, is_active, unique_id) VALUES (?, ?, ?, ?)";
                $stmt = $mysqli->prepare($insert_sql);
                $stmt->bind_param("ssss", $district_name, $taluk_name, $is_active, $columns['unique_id']);
            }
    ...
    case 'datatable':
        ...
        if ($result) {
            ...
            foreach ($res_array as $key => $value) {
                ...
                $value['zone_name'] = disname($value['zone_name']);
