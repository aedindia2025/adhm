...
switch ($action) {
    case 'createupdate':
        ...
        $select_sql = 'SELECT COUNT(unique_id) AS count FROM ' . $table . ' WHERE user_type = ? AND is_delete = 0';
        $params = ['s', $user_type];
        ...
        if ($stmt) {
            ...
            if ($count > 0) {
                ...
                if ($unique_id) {
                    // Update record
                    $update_sql = 'UPDATE ' . $table . ' SET user_type = ?, is_active = ? WHERE unique_id = ?';
                    $update_params = ['sss', $user_type, $is_active, $unique_id];
                    ...
                    // Insert record
                    $insert_sql = 'INSERT INTO ' . $table . ' (user_type, is_active, unique_id) VALUES (?, ?, ?)';
                    $insert_params = ['sss', $user_type, $is_active, unique_id($prefix)];
                    $stmt = $mysqli->prepare($insert_sql);
                    if ($stmt) {
                        $stmt->bind_param(...$insert_params);
                        $status = $stmt->execute();
                        $stmt->close();
                    }
                }
        ...
        case 'datatable':
            ...
            if ($stmt) {
                ...
                $stmt->bind_result($s_no, $user_type, $is_active, $unique_id);
