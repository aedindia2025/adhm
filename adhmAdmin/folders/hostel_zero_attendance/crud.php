...
switch ($action) {
    ...
    case 'datatable':
        ...
        // Define columns to match your SELECT
        $columns = [
            "@a:=@a+1 as s_no",
            ...
            "h.sanctioned_strength"
        ];

        // Table definition (hostel_name as main table)
        // $table_details = [
        //     "hostel_name h
        //  LEFT JOIN attendance_report a 
        //  ON a.hostel_unique_id = h.unique_id
        //  AND a.report_date = '" . $_POST['from_date'] . "'
        //  AND a.report_date = '" . $_POST['to_date'] . "',
        // (SELECT @a:=0) AS a",
        //     $columns
        // ];
        ...
        // Base WHERE condition: only active hostels with NO attendance entry
        $where = "h.is_delete = 0 AND h.is_active = 1 AND a.hostel_unique_id IS NULL";

        // Filters
        if (!empty($_POST['district_name'])) {
            $where .= " AND h.district_name = '" . $_POST['district_name'] . "'";
        }

        if (!empty($_POST['taluk_name'])) {
            $where .= " AND h.taluk_name = '" . $_POST['taluk_name'] . "'";
        }

        if (!empty($_POST['hostel_name'])) {
            $where .= " AND h.unique_id = '" . $_POST['hostel_name'] . "'";
        }

        // Optional search filter
        if (!empty($search)) {
            $where .= " AND (h.hostel_name LIKE '%$search%' OR h.unique_id LIKE '%$search%')";
        }

        $order_by = "";
        $group_by = "h.unique_id";
        ...
        if ($result->status) {
            ...
            foreach ($res_array as $value) {
                $btn_update = btn_update($folder_name, $value['hostel_unique_id']);
                $btn_delete = btn_delete($folder_name, $value['hostel_unique_id']);

                $value['actions'] = $btn_update . $btn_delete;
