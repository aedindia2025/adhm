...
switch ($action) {
    ...
    case 'createupdate':
        ...
        $prefix = "prefix_value"; // Define your prefix value here
        ...
        $select_query = "SELECT COUNT(unique_id) AS count FROM $table WHERE disbursement_type = ? AND is_delete = 0";
        if ($unique_id) {
        ...
        if ($unique_id) {
            $stmt->bind_param('ss', $disbursement_type, $unique_id);
        } else {
            $stmt->bind_param('s', $disbursement_type);
        }
    ...
    case 'datatable':
        ...
        if ($result) {
            ...
            foreach ($res_array as $key => $value) {
                $value['disbursement_type'] = disname($value['disbursement_type']);
