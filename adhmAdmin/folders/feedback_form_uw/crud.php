...
switch ($action) {
    case 'createupdate':
        ...
        $select_where       = 'feedback_type = "'.$feedback_type.'"  AND is_delete = 0  ';
    ...
    case 'datatable':
        ...
        if ($_POST['search']['value']) {
           $where .= " AND feedback_type LIKE '".mysql_like($_POST['search']['value'])."' ";
        }
        ...
        if ($result->status) {
            ...
            foreach ($res_array as $key => $value) {
                // $value['feedback_type'] = disname($value['feedback_type']);
