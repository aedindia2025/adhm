...
switch ($action) {
  ...
  case 'datatable':
    ...
    $columns = [
        ...
        "std_app_no",
        "std_name",
        "(SELECT hostel_id FROM hostel_name WHERE unique_id = std_reg_s.hostel_1) as hostel_id",
        "(SELECT hostel_name FROM hostel_name WHERE unique_id = std_reg_s.hostel_1) as hostel_name",
    ...
    $table_details = [
        ...
        "std_reg_s, (SELECT @a:=0) AS a",
        $columns
    ];

    // Base where clause
    $where = "is_delete = 0 AND std_to_inst_distance <= 5 ";
