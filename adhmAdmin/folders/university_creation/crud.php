...
switch ($action) {
    ...
    case 'createupdate':
        ...
        $select_query = "SELECT COUNT(unique_id) AS count FROM $table WHERE university_name = ? AND stream_unique_id = ? AND is_delete = 0";
        ...
        if ($unique_id) {
            $stmt->bind_param('sss', $university_name, $stream_name, $unique_id);
        } else {
            $stmt->bind_param('ss', $university_name, $stream_name);
        }
        ...
        if ($data['count']) {
            ...
            if ($unique_id) {
                ...
                $update_query = "UPDATE $table SET stream_unique_id = ?, university_name = ?, is_active = ? WHERE unique_id = ?";
                $stmt = $mysqli->prepare($update_query);
                $stmt->bind_param('ssss', $stream_name, $university_name, $is_active, $unique_id);
            } else {
                // Insert new record
                $insert_query = "INSERT INTO $table (stream_unique_id, university_name, is_active, unique_id) VALUES (?, ?, ?, ?)";
                $stmt = $mysqli->prepare($insert_query);
                $stmt->bind_param('ssss', $stream_name, $university_name, $is_active, unique_id($prefix));
            }
