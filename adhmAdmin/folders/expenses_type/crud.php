...
switch ($action) {
    case 'createupdate':
        ...
        $expenses_type = sanitizeInput($_POST['expenses_type']);
        $is_active = sanitizeInput($_POST['is_active']);
        $unique_id = sanitizeInput($_POST['unique_id']);

        // Check if the entry already exists
        $select_sql = "SELECT COUNT(unique_id) AS count FROM $table WHERE expenses_type = ? AND is_delete = ?";
        if ($unique_id) {
        ...
        if ($unique_id) {
            $stmt->bind_param('sss', $expenses_type, $is_delete, $unique_id);
        } else {
            $stmt->bind_param('ss', $expenses_type, $is_delete);
        }
        ...
        if ($data['count'] > 0) {
            ...
            if ($unique_id) {
                $update_sql = "UPDATE $table SET expenses_type = ?, is_active = ? WHERE unique_id = ?";
                $stmt = $mysqli->prepare($update_sql);
                $stmt->bind_param('sss', $expenses_type, $is_active, $unique_id);
            } else {
                $columns = [
                ...
                $insert_sql = "INSERT INTO $table (expenses_type, is_active, unique_id) VALUES (?, ?, ?)";
                $stmt = $mysqli->prepare($insert_sql);
                $stmt->bind_param('sss', $expenses_type, $is_active, $columns['unique_id']);
            }
    ...
    case 'datatable':
        ...
        if ($result) {
            ...
            foreach ($res_array as $key => $value) {
                $value['expenses_type'] = disname($value['expenses_type']);
