...
switch ($action) {
    ...
    case 'get_hostel_by_taluk_name':
        ...
        case 'get_chart_data':
            $json_array = "";
            $columns = [
                "(SELECT COUNT(unique_id) FROM std_app_s WHERE student_type = '65f00a259436412348') AS school_count",
                "(SELECT COUNT(unique_id) FROM std_app_s WHERE student_type = '65f00a327c08582160') AS iti_count ",
                "(SELECT COUNT(unique_id) FROM std_app_s WHERE student_type = '65f00a495599589293') AS college_ug_count",
                "(SELECT COUNT(unique_id) FROM std_app_s WHERE student_type = '65f00a53eef3015995') AS college_pg_count",
                "(SELECT COUNT(unique_id) FROM std_app_s WHERE student_type = '65f00a3e3c9a337012') AS diplomo_count",
            ];
            $table_details = [
                "std_app_s",
                $columns
            ...
            case 'pending_application_chart_data':
                $json_array = "";
                $columns = [
                    "(SELECT COUNT(id) as count FROM std_app_s WHERE  is_delete = 0 AND application_type = 1) AS  new_count", 
                    "(SELECT COUNT(id) as count FROM std_app_s WHERE  is_delete = 0 AND application_type = 2) AS renewal_count" 
                ];
                $table_details = [
                    "std_app_s",
                    $columns
                ...
                case 'boys_girls_count':
                    ...
                    $json_array = [];
                    $columns = [
                        "(SELECT COUNT(id) FROM aadhar WHERE aadhar.agender = 'M') as boys",
                        "(SELECT COUNT(id) FROM aadhar WHERE aadhar.agender = 'F') as girls"
                    ...
                    case 'new_renewal_count':
                        ...
                        case 'get_student_count':
                            ...
                            $where = "is_delete = 0 and submit_status= 1 and hostel_district_1 != '' GROUP BY hostel_district_1 ORDER BY cnt DESC";
                        ...
                        case 'get_processing_times':
                            ...
                            $columns = [
                                ...
                                "(SELECT AVG(DATEDIFF(status_upd_date, DATE(created))) AS avg_time FROM std_app_s WHERE application_type = 1 AND status_upd_date IS NOT NULL) AS new_time",
                                "(SELECT AVG(DATEDIFF(status_upd_date, DATE(created))) AS avg_time FROM std_app_s WHERE application_type = 2 AND status_upd_date IS NOT NULL) AS renewal_time"
    ...
    case 'total_staff_strength':
        ...
        //     $columns = [
        //         "dn.district_name",
        //         "COUNT(sr.unique_id) AS total_capacity"
        //     ];
        ...
        //     // Join condition
        //     $join_condition = "JOIN std_reg_s sr ON sr.hostel_district_1 = dn.unique_id";
            
        //     // Where condition
        //     $where = "sr.is_delete = 0";
            
        //     // Group by clause
        //     $group_by = "GROUP BY dn.district_name";
            
        //     // Construct the complete SQL query
        //     $sql = "SELECT " . implode(", ", $columns) . " FROM " . $table_details[0] . " " . $join_condition . " WHERE " . $where . " " . $group_by;
            
        //     try {
        //         // Execute the query using the PDO select method
        //         $result = $pdo->select($table_details, $where, $join_condition, $group_by);
...
// $week=1;
//     $entry_date[]=$week++;                  
    $new_cnt[] = week_reg($startOfWeek, $endOfWeek, '1');
    $renewal_cnt[] = week_reg($startOfWeek, $endOfWeek, '2');
...
// echo $renewal_cnt;
            ...
            $json_array = [
                ...
                "entry_date" => ['1st week','2nd week','3rd week','4th week'],
                "new_cnt" => $new_cnt,
                "renewel" => $renewal_cnt
...
foreach ($dates as $date) {
        ...
        $explode = explode('-', $date);
                            $firstPart = $explode[2];
           $entry_date=['1st day','2nd day','3rd day','4th day','5th day','6th day','7th day'];     

           $new_cnt[] = reg_register($date,'1');
           $renewal_cnt[] = reg_register($date,'2');
...
// echo $json_array;
    ...
    case 'custome_wise_counts':
        ...
        while ($currentYear < $to_year || ($currentYear == $to_year && $currentWeek <= $to_week)) {
            ...
            $new_cnt[] = cus_reg($startDate, $endDate,'1');
            $renewal_cnt[] = cus_reg($startDate, $endDate,'2');


    
            // Add data for this week
            $entry_date[] = $s_no; // Append serial number for this week
...
// // Extract year and week from from_date
// list($from_year, $from_week) = explode('-W', $from_date);
// list($to_year, $to_week) = explode('-W', $to_date);
...
//     $reg_district = cus_reg($startDate,$endDate);
//     $s_no++;
//     // echo "gowshoojiii";
// }
