...
$excel->getActiveSheet()
    ...
    ->setCellValue('G3', 'IFHRMS ID')
    ->setCellValue('H3', 'Staff Name')
    ->setCellValue('I3', 'Designation')
...
$establishment_type = isset($_GET['establishment_type']) ? $_GET['establishment_type'] : '';
...
if ($establishment_type) {
    $whereClauses[] = "designation_un = :establishment_type";
}
...
$sql = "SELECT 
            ...
            hostel_name,
            designation
        FROM staff_attendance_report" . $whereSQL;
...
if ($establishment_type) {
    $statement->bindParam(':establishment_type', $establishment_type);
}
...
foreach ($users as $row) {
    ...
    $excel->getActiveSheet()
        ...
        ->setCellValue('H' . $row_val, $row["userName"] ?: '-')
        ->setCellValue('I' . $row_val, $row["designation"] ?: '-')
