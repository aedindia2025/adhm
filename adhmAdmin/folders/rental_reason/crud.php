...
switch ($action) {
    case 'createupdate':
        ...
        $rental_reason = sanitizeInput($_POST["rental_reason"]);
        $is_active = sanitizeInput($_POST["is_active"]);
        $unique_id = sanitizeInput($_POST["unique_id"]);


        // Check if the district already exists
        $select_sql = "SELECT COUNT(unique_id) AS count FROM $table WHERE rental_reason = ? AND is_delete = 0";
        if ($unique_id) {
        ...
        if ($unique_id) {
            $stmt->bind_param("ss", $rental_reason, $unique_id);
        } else {
            $stmt->bind_param("s", $rental_reason);
        }
        ...
        if ($data["count"] > 0) {
            ...
            if ($unique_id) {
                ...
                $update_sql = "UPDATE $table SET rental_reason = ?, is_active = ? WHERE unique_id = ?";
               
                $stmt = $mysqli->prepare($update_sql);
                $stmt->bind_param("sss", $rental_reason, $is_active, $unique_id);
            } else {
                $columns = [
                    "rental_reason" => $rental_reason,
                    "is_active" => $is_active,
                ...
                $insert_sql = "INSERT INTO $table (rental_reason, is_active, unique_id) VALUES (?, ?, ?)";
                $stmt = $mysqli->prepare($insert_sql);
                $stmt->bind_param("sss", $rental_reason, $is_active, $columns['unique_id']);
            }
