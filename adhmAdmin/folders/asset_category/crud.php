...
switch ($action) {
    case 'createupdate':
        ...
        $sql = 'SELECT COUNT(unique_id) AS count FROM ' . $table . ' WHERE asset_category = ? AND is_delete = 0';
        if ($unique_id) {
            $sql .= ' AND unique_id != ?';
        }
        ...
        if ($unique_id) {
            $stmt->bind_param("ss", $asset_category, $unique_id);
        } else {
            $stmt->bind_param("s", $asset_category);
        }
	    ...
	    $status = true;
            ...
            if ($unique_id) {
                // Perform update
                $sql = 'UPDATE ' . $table . ' SET asset_category = ?, description = ?, is_active = ? WHERE unique_id = ?';
                $stmt = $mysqli->prepare($sql);
                $stmt->bind_param("ssis", $asset_category, $description, $is_active, $unique_id);
            } else {
                // Perform insert
                $sql = 'INSERT INTO ' . $table . ' (asset_category, description, is_active, unique_id) VALUES (?, ?, ?, ?)';
                $stmt = $mysqli->prepare($sql);
                $stmt->bind_param("ssis", $asset_category, $description, $is_active, $columns["unique_id"]);
            }
