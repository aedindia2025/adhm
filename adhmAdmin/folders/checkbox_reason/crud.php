...
switch ($action) {
    case 'createupdate':
        ...
        $select_sql = "SELECT COUNT(unique_id) AS count FROM $table WHERE reason = ? AND is_delete = 0";
        if ($unique_id) {
        ...
        if ($unique_id) {
            $stmt->bind_param("ss", $reason, $unique_id);
        } else {
        ...
        if ($data["count"] > 0) {
            ...
            if ($unique_id) {
                ...
                $update_sql = "UPDATE $table SET reason = ?, is_active = ? WHERE unique_id = ?";
                $stmt = $mysqli->prepare($update_sql);
                $stmt->bind_param("sss", $reason, $is_active, $unique_id);
            } else {
                $columns = [
                    "reason" => $reason,
                    "is_active" => $is_active,
                ...
                $insert_sql = "INSERT INTO $table (reason, is_active, unique_id) VALUES (?, ?, ?)";
                $stmt = $mysqli->prepare($insert_sql);
                $stmt->bind_param("sss", $reason, $is_active, $columns['unique_id']);
            }
