...
switch ($action) {
    case 'createupdate':
        ...
        $ownership = sanitizeInput($_POST["ownership"]);
        $is_active = sanitizeInput($_POST["is_active"]);
        $unique_id = sanitizeInput($_POST["unique_id"]);


        // Check if the district already exists
        $select_sql = "SELECT COUNT(unique_id) AS count FROM $table WHERE ownership = ? AND is_delete = 0";
        if ($unique_id) {
        ...
        if ($unique_id) {
            $stmt->bind_param("ss", $ownership, $unique_id);
        } else {
            $stmt->bind_param("s", $ownership);
        }
        ...
        if ($data["count"] > 0) {
            ...
            if ($unique_id) {
                ...
                $update_sql = "UPDATE $table SET ownership = ?, is_active = ? WHERE unique_id = ?";
                $stmt = $mysqli->prepare($update_sql);
                $stmt->bind_param("sss", $ownership, $is_active, $unique_id);
            } else {
                $columns = [
                    "ownership" => $ownership,
                    "is_active" => $is_active,
                ...
                $insert_sql = "INSERT INTO $table (ownership, is_active, unique_id) VALUES (?, ?, ?)";
                $stmt = $mysqli->prepare($insert_sql);
                $stmt->bind_param("sss", $ownership, $is_active, $columns['unique_id']);
            }
