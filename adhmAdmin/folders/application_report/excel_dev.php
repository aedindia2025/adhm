...
foreach ($users as $row) {
    ...
    switch ($row['student_type']) {
        ...
        case '65f00a495599589293':
            $hostel_type = 'College-UG';
            break;
        case '65f00a53eef3015995':
            $hostel_type = 'College-PG';
            break;
        default:
            $hostel_type = '-'; // If no match, set to '-'
    }
    ...
    if ($row['dropout_status'] != '' || $row['dropout_status'] != NULL) {
        ...
        if ($row['dropout_status'] == '0') {
            $dropout_status = 'Pending';
            $row['dropout_reject_reason'] = '-';
        } elseif ($row['dropout_status'] == '1') {
            $dropout_status = 'Approved';
            $row['dropout_reject_reason'] = '-';
