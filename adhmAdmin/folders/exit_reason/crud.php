...
switch ($action) {
    case 'createupdate':
        ...
        $exit_reason = sanitizeInput($_POST["exit_reason"]);
        $is_active = sanitizeInput($_POST["is_active"]);
        $unique_id = sanitizeInput($_POST["unique_id"]);


        // Check if the district already exists
        $select_sql = "SELECT COUNT(unique_id) AS count FROM $table WHERE exit_reason = ? AND is_delete = 0";
        if ($unique_id) {
        ...
        if ($unique_id) {
            $stmt->bind_param("ss", $exit_reason, $unique_id);
        } else {
            $stmt->bind_param("s", $exit_reason);
        }
        ...
        if ($data["count"] > 0) {
            ...
            if ($unique_id) {
                ...
                $update_sql = "UPDATE $table SET exit_reason = ?, is_active = ? WHERE unique_id = ?";
                $stmt = $mysqli->prepare($update_sql);
                $stmt->bind_param("sss", $exit_reason, $is_active, $unique_id);
            } else {
                $columns = [
                    "exit_reason" => $exit_reason,
                    "is_active" => $is_active,
                ...
                $insert_sql = "INSERT INTO $table (exit_reason, is_active, unique_id) VALUES (?, ?, ?)";
                $stmt = $mysqli->prepare($insert_sql);
                $stmt->bind_param("sss", $exit_reason, $is_active, $columns['unique_id']);
            }
