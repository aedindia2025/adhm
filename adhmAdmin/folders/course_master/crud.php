...
switch ($action) {
    case 'createupdate':
        ...
        $select_sql = "SELECT COUNT(unique_id) AS count FROM $table WHERE course_name = ? AND is_delete = 0";
        if ($unique_id) {
        ...
        if ($unique_id) {
            $stmt->bind_param("ss", $course_name, $unique_id);
        } else {
        ...
        if ($data["count"] > 0) {
            ...
            if ($unique_id) {
                $update_sql = "UPDATE $table SET hostel_type = ?, course_name = ?, is_active = ? WHERE unique_id = ?";
                $stmt = $mysqli->prepare($update_sql);
                $stmt->bind_param("ssss", $hostel_type, $course_name, $is_active, $unique_id);
            } else {
                $columns = [
                ...
                $insert_sql = "INSERT INTO $table (hostel_type, course_name, is_active, unique_id) VALUES (?, ?, ?, ?)";
                $stmt = $mysqli->prepare($insert_sql);
                $stmt->bind_param("ssss", $hostel_type, $course_name, $is_active, $columns['unique_id']);
            }
    ...
    case 'datatable':
        ...
        if ($result) {
            ...
            foreach ($res_array as $key => $value) {
                ...
                // Replace numeric hostel_type with label
                if (isset($hostel_type_map[$value['hostel_type']])) {
                    $value['hostel_type'] = $hostel_type_map[$value['hostel_type']];
                }
                $value['course_name'] = disname($value['course_name']);
