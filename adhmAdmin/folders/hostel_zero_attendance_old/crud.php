...
switch ($action) {
    ...
    case 'datatable':
        ...
        $table_details = [
            "hostel_name h 
         LEFT JOIN attendance_report s ON s.hostel_name = h.unique_id,
         (SELECT @a:=0) AS a",
            $columns
        ];

        // WHERE condition: only hostels not assigned to any student
        $where = "s.id IS NULL AND h.is_delete = 0 AND h.hostel_id NOT LIKE '%ADWN%'";

        // Filter conditions
        // if ($_POST['from_date'] && $_POST['to_date']) {
        //     $where = 's.report_date >= "' . $_POST['from_date'] . '" and s.report_date <= "' . $_POST['to_date'] . '"';
        // } else {
        //     if ($_POST['from_date']) {
        //         $where = 's.report_date >= "' . $_POST['from_date'] . '"';
        //     }
        //     if ($_POST['to_date']) {
        //         $where = 's.report_date <= "' . $_POST['to_date'] . '"';
        //     }
        // }
        ...
        if (!empty($_POST['hostel_name'])) {
            $where .= " AND h.hostel_name = '" . $_POST['hostel_name'] . "'";
        }
