...
switch ($action) {
    ...
    case 'createupdate':
        ...
        $select_query = "SELECT COUNT(unique_id) AS count FROM $table WHERE stream_unique_id = ? AND university_unique_id = ? AND college_name = ? AND is_delete = 0";
        ...
        if ($unique_id) {
            $stmt->bind_param('ssss', $stream_name, $university_name, $college_name, $unique_id);
        } else {
            $stmt->bind_param('sss', $stream_name, $university_name, $college_name);
        }
        ...
        if ($data['count']) {
            ...
            if ($unique_id) {
                ...
                $update_query = "UPDATE $table SET stream_unique_id = ?, university_unique_id = ?, college_name = ?, is_active = ? WHERE unique_id = ?";
                $stmt = $mysqli->prepare($update_query);
                $stmt->bind_param('sssss', $stream_name, $university_name, $college_name, $is_active, $unique_id);
            } else {
                // Insert new record
                $insert_query = "INSERT INTO $table (stream_unique_id, university_unique_id, college_name, is_active, unique_id) VALUES (?, ?, ?, ?, ?)";
                $stmt = $mysqli->prepare($insert_query);
                $stmt->bind_param('sssss', $stream_name, $university_name, $college_name, $is_active, unique_id($prefix));
            }
    ...
    case 'datatable':
        ...
        $columns = [
            "@a:=@a+1 s_no",
            "(SELECT stream_type FROM stream_type AS stream_type WHERE stream_type.unique_id = " . $table . ".stream_unique_id ) AS stream_type",
            "(SELECT university_name FROM university_creation WHERE university_creation.unique_id = " . $table . ".university_unique_id ) AS university_name",
            "college_name",
            "is_active",
            "unique_id"
        ];
