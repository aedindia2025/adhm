...
switch ($action) {
    case 'createupdate':
        ...
        $vali_stream_type = filter_input(INPUT_POST, 'stream_type', FILTER_SANITIZE_STRING);
        $vali_ip = filter_input(INPUT_POST, 'ip', FILTER_SANITIZE_STRING);
        $vali_user_name = filter_input(INPUT_POST, 'user_name', FILTER_SANITIZE_STRING);
        $vali_d_password = filter_input(INPUT_POST, 'd_password', FILTER_SANITIZE_STRING);
        // $vali_gender_type = filter_input(INPUT_POST,'gender_type',FILTER_SANITIZE_STRING);

        if (!$vali_district_name || !$vali_taluk_name || !$vali_hostel_name || !$vali_academic_year || !$vali_cam_name || !$vali_link1 || !$vali_link2 || !$vali_stream_type || !$vali_ip || !$vali_user_name || !$vali_d_password ) {
            $msg = "form_alert";
        } else {
    ...
    case 'cctv_list':
        ...
        if ($taluk_name) {
            $where .= "taluk_name = '$taluk_name' and ";
        }
        if ($hostel_name) {
            $where .= "hostel_name = '$hostel_name' and ";
        }
        if ($academic_year) {
            $where .= "academic_year = '$academic_year' and ";
        }

$where .= 'is_delete = 0 order by FIELD(status, 1,2,0), id ASC';
        ...
        if ($result->status) {
            ...
            foreach ($res_array as $key => $value) {
                   ...
                   if($value['status'] == '1'){ 
                    ...
                    $play = '<button class="btn btn-primary action-button" onclick="get_video(\'' .$value['unique_id']. '\')">Play</button><br>';
...
$play = '<button class="btn btn-primary action-button" onclick="get_video(\'' .$value['unique_id']. '\')">Play</button><br>';
                ...
                $card_html = '<br> <h5>' . htmlspecialchars($value['hostel_id']) . '</h4>    
                        <h5>' . htmlspecialchars($value['hostel_name']) . '</h5>
                        '.$status.$play.'<hr>';
            ...
            if (count($results->data) == '0') {
                ...
                foreach ($res_array as $key => $value) {
                    ...
                    if($value['cam_link1'] != ''){
                        ...
                        $cam_1_content = '<iframe width="100%" height="300" src="https://nallosaims.tn.gov.in:5443/'.$value['stream_type'].'/play.html?id=' . $value['cam_link1'] . '" frameborder="0" allowfullscreen></iframe>';

                        // $cam_1_content = '<iframe width="560" height="315" src="https://nallosaims.tn.gov.in:5443/WebRTCApp/play.html?id=TNADW01001_01" frameborder="0" allowfullscreen></iframe>';

                        $cam_1_header = '<div class="card-header"><b>District Name:</b> ' . $value['district_name'] . ' &nbsp;&nbsp;&nbsp;&nbsp; <b>Taluk Name:</b> ' . $value['taluk_name'] . ' &nbsp;&nbsp;&nbsp;&nbsp; <b>Hostel Name:</b> ' . $value['hostel_name'] . '&nbsp;&nbsp;&nbsp;&nbsp;<b>Hostel ID:</b>' . $value['hostel_id'] . '</div><hr style="border-color: #00000029;">';

                        $cam_1 .= '<div style="width: 100%; margin: 10px; background-color: #e2e2e2;">'
                        . $cam_1_header
                        . '<div class="card-body">'
                        . $cam_1_content
                        . '</div></div>';

                        $cam1 = '<div style="display: flex; flex-wrap: wrap;"><form method="GET">' . $cam_1 . '</form></div>';

                    }

                    if($value['cam_link2'] != ''){
                       
                        $cam_2_content = '<iframe width="100%" height="300" src="https://nallosaims.tn.gov.in:5443/'.$value['stream_type'].'/play.html?id=' . $value['cam_link2'] . '" frameborder="0" allowfullscreen></iframe>';

                        // $cam_2_content = '<iframe width="100%" height="300" src="https://nallosaims.tn.gov.in:5443/WebRTCApp/play.html?id=TNADW01001_02" frameborder="0" allowfullscreen></iframe>';

                        $cam_2_header = '<div class="card-header"><b>District Name:</b> ' . $value['district_name'] . ' &nbsp;&nbsp;&nbsp;&nbsp; <b>Taluk Name:</b> ' . $value['taluk_name'] . ' &nbsp;&nbsp;&nbsp;&nbsp; <b>Hostel Name:</b> ' . $value['hostel_name'] . '&nbsp;&nbsp;&nbsp;&nbsp;<b>Hostel ID:</b>' . $value['hostel_id'] . '</div><hr style="border-color: #00000029;">';

                        $cam_2 .= '<div style="width: 100%; margin: 10px; background-color: #e2e2e2;">'
                        . $cam_2_header
                        . '<div class="card-body">'
                        . $cam_2_content
                        . '</div></div>';

                        $cam2 = '<div style="display: flex; flex-wrap: wrap;"><form method="GET">' . $cam_2 . '</form></div>';
