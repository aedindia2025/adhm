...
switch ($action) {
    ...
    case 'createupdate':
        ...
        $columns = [
            "district_unique_id" => $district_name,
            "taluk_unique_id" => $taluk_name,
        ...
        $select_query = "SELECT COUNT(unique_id) AS count FROM $table WHERE district_unique_id = ? AND taluk_unique_id = ? AND school_name = ? AND is_delete = 0";
        ...
        if ($unique_id) {
            $stmt->bind_param('ssss', $district_name, $taluk_name, $school_name, $unique_id);
        } else {
            $stmt->bind_param('sss', $district_name, $taluk_name, $school_name);
        }
        ...
        if ($data['count']) {
            ...
            if ($unique_id) {
                ...
                $update_query = "UPDATE $table SET district_unique_id = ?, taluk_unique_id = ?, school_name = ?, is_active = ? WHERE unique_id = ?";
                $stmt = $mysqli->prepare($update_query);
                $stmt->bind_param('sssis', $district_name, $taluk_name, $school_name, $is_active, $unique_id);
            } else {
                // Insert new record
                $insert_query = "INSERT INTO $table (district_unique_id, taluk_unique_id, school_name, is_active, unique_id) VALUES (?, ?, ?, ?, ?)";
                $stmt = $mysqli->prepare($insert_query);
                $stmt->bind_param('sssis', $district_name, $taluk_name, $school_name, $is_active, $columns['unique_id']);
            }
    ...
    case 'datatable':
        ...
        if ($result) {
            ...
            foreach ($res_array as $key => $value) {
                ...
                // $value['taluk_name']     = $taluk_name [0]['$taluk_name'];
