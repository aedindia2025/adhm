...
switch ($action) {
    case 'createupdate':
        ...
        $assembly_const_name = sanitizeInput($_POST["assembly_const_name"]);
        ...
        $columns = [
            ...
            "assembly_const_name" => $assembly_const_name,
            "is_active" => $is_active,
            "unique_id" => unique_id($prefix)
        ];

        // check already Exist Or not
        $select_where = 'assembly_const_name = ? AND is_delete = 0';
        ...
        if ($unique_id) {
            $stmt->bind_param('ss', $assembly_const_name, $unique_id);
        } else {
            $stmt->bind_param('s', $assembly_const_name);
        }
        $stmt->execute();
        ...
        // } else {
            if ($unique_id) {
                ...
                $stmt = $mysqli->prepare("UPDATE $table SET district_name = ?,assembly_const_name = ?, is_active = ? WHERE unique_id = ?");
                $stmt->bind_param('ssss', $district_name,  $assembly_const_name, $is_active, $unique_id);
            } else {
                
                $stmt = $mysqli->prepare("INSERT INTO $table (district_name, assembly_const_name, is_active, unique_id) VALUES (?, ?, ?, ?)");
                $stmt->bind_param('ssss', $district_name,  $assembly_const_name, $is_active, $columns['unique_id']);
            }
            if ($stmt->execute()) {
                $status = true;
                if ($unique_id) {
