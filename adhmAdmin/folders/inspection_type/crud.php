<?php

// Get folder Name From Currnent Url 
$folder_name = explode("/", $_SERVER['PHP_SELF']);
$folder_name = $folder_name[count($folder_name) - 2];

// // Database Country Table Name
$table = "inspection_type";

// // Include DB file and Common Functions
include '../../config/dbconfig.php';

// // Variables Declaration
$action = $_POST['action'];

$fund_name = "";
$is_active = "";
$unique_id = "";
$prefix = "";

$data = "";
$msg = "";
$error = "";
$status = "";
$test = ""; // For Developer Testing Purpose

function validateCSRFToken($token)
{
    return isset($_SESSION['csrf_token']) && hash_equals($_SESSION['csrf_token'], $token);
}

switch ($action) {

    case 'createupdate':

        $token = $_POST['csrf_token'];
        if (!validateCSRFToken($token)) {
            die('CSRF validation failed.');
        }

        $inspections_type = sanitizeInput($_POST["inspections_type"]);
        $description = sanitizeInput($_POST["description"]);
        $is_active = sanitizeInput($_POST["is_active"]);
        $unique_id = sanitizeInput($_POST["unique_id"]);

        $update_where = "";

        // Check if already exists
        $select_query = 'SELECT COUNT(unique_id) AS count FROM ' . $table . ' WHERE inspections_type = ? AND is_delete = 0';
        if ($unique_id) {
            $select_query .= ' AND unique_id != ?';
        }

        if ($stmt = $mysqli->prepare($select_query)) {
            if ($unique_id) {
                $stmt->bind_param("ss", $inspections_type, $unique_id);
            } else {
                $stmt->bind_param("s", $inspections_type);
            }

            $stmt->execute();
            $stmt->bind_result($count);
            $stmt->fetch();
            $stmt->close();
        } else {
            echo json_encode([
                "status" => false,
                "data" => [],
                "error" => $mysqli->error,
                "msg" => "error"
            ]);
            break;
        }

        if ($count) {
            $msg = "already";
            $status = true;
        } else {
            $columns = [
                "inspections_type" => $inspections_type,
                "description" => $description,
                "is_active" => $is_active
            ];

            if ($unique_id) {
                // Update existing record
                $update_query = 'UPDATE ' . $table . ' SET inspections_type = ?, description = ?, is_active = ? WHERE unique_id = ?';
                if ($stmt = $mysqli->prepare($update_query)) {
                    $stmt->bind_param("ssss", $inspections_type, $description, $is_active, $unique_id);
                    $status = $stmt->execute();
                    $stmt->close();
                } else {
                    echo json_encode([
                        "status" => false,
                        "data" => [],
                        "error" => $mysqli->error,
                        "msg" => "error"
                    ]);
                    break;
                }
                $msg = $status ? "update" : "error";
            } else {
                // Insert new record
                $unique_id = unique_id($prefix);
                $insert_query = 'INSERT INTO ' . $table . ' (inspections_type, description, is_active, unique_id) VALUES (?, ?, ?, ?)';
                if ($stmt = $mysqli->prepare($insert_query)) {
                    $stmt->bind_param("ssss", $inspections_type, $description, $is_active, $unique_id);
                    $status = $stmt->execute();
                    $stmt->close();
                } else {
                    echo json_encode([
                        "status" => false,
                        "data" => [],
                        "error" => $mysqli->error,
                        "msg" => "error"
                    ]);
                    break;
                }
                $msg = $status ? "create" : "error";
            }
        }

        $json_array = [
            "status" => $status,
            "data" => [],
            "error" => $status ? "" : $mysqli->error,
            "msg" => $msg
        ];

        echo json_encode($json_array);

        break;

    case 'datatable':
        // DataTable Variables
        $length = $_POST['length'];
        $start = $_POST['start'];
        $draw = $_POST['draw'];
        $limit = $length;

        $data = [];


        if ($length == '-1') {
            $limit = "";
        }

        // Query Variables
        $json_array = "";
        $columns = [
            "@a:=@a+1 s_no",
            "inspections_type",
            "description",
            "is_active",
            "unique_id"
        ];
        $table_details = "$table , (SELECT @a:= ?) AS a";
        $where = "is_delete = 0";
        $bind_params = "i"; // Types of parameters (i for integer)
        $bind_values = [$start];


        $sql_function = "SQL_CALC_FOUND_ROWS";
        $order_by = ""; // Set the order by clause if necessary

        // SQL query for data fetching
        $sql = "SELECT $sql_function " . implode(", ", $columns) . " FROM $table_details WHERE $where";
        if ($limit !== "") {
            $sql .= " LIMIT ?, ?";
            $bind_params .= "ii"; // Add types for limit parameters
            $bind_values[] = $start;
            $bind_values[] = $limit;
        }

        $stmt = $mysqli->prepare($sql);

        if ($stmt === false) {
            // Handle prepare error
            $json_array = [
                "draw" => intval($draw),
                "recordsTotal" => 0,
                "recordsFiltered" => 0,
                "data" => [],
                "error" => $mysqli->error
            ];
            echo json_encode($json_array);
            break;
        }

        // Bind parameters dynamically
        $bind_params_arr = array_merge([$bind_params], ...array_map(function ($v) {
            return [$v];
        }, $bind_values));
        call_user_func_array([$stmt, 'bind_param'], $bind_params_arr);

        $stmt->execute();
        $result = $stmt->get_result();

        // Fetch total records
        $total_records_result = $mysqli->query("SELECT FOUND_ROWS() as total");
        $total_records = $total_records_result->fetch_assoc()['total'];
        if ($result) {

            $res_array = $result->fetch_all(MYSQLI_ASSOC);

            foreach ($res_array as $key => $value) {
                $value['fund_name'] = disname($value['fund_name']);
                $value['is_active'] = is_active_show($value['is_active']);


                $btn_update = btn_update($folder_name, $value['unique_id']);
                $btn_delete = btn_delete($folder_name, $value['unique_id']);

                if ($value['unique_id'] == "5f97fc3257f2525529") {
                    $btn_update = "";
                    $btn_delete = "";
                }

                $value['unique_id'] = $btn_update . $btn_delete;
                $data[] = array_values($value);
            }

            $json_array = [
                "draw" => intval($draw),
                "recordsTotal" => intval($total_records),
                "recordsFiltered" => intval($total_records),
                "data" => $data,
                // "testing"           => $result->sql
            ];
        } else {
            // print_r($result);
        }

        echo json_encode($json_array);
        break;


    case 'delete':

        $token = $_POST['csrf_token'];
        if (!validateCSRFToken($token)) {
            die('CSRF validation failed.');
        }
        $unique_id = $_POST['unique_id'];

        // Prepare SQL statement
        $sql = "UPDATE $table SET is_delete = 1 WHERE unique_id = ?";
        $stmt = $mysqli->prepare($sql);
        $stmt->bind_param("s", $unique_id);

        // Execute SQL statement
        if ($stmt->execute()) {
            $status = "success";
            $msg = "success_delete";
        } else {
            $status = "error";
            $msg = "error";
        }

        $stmt->close();

        $json_array = [
            "status" => $status,
            "msg" => $msg,
        ];

        echo json_encode($json_array);
        break;

    default:

        break;
}

?>