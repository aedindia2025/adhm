...
switch ($action) {
    case 'createupdate':
        ...
        $select_sql = "SELECT COUNT(unique_id) AS count FROM $table WHERE district_name = ? AND is_delete = 0";
        if ($unique_id) {
        ...
        if ($unique_id) {
            $stmt->bind_param("ss", $district_name, $unique_id);
        } else {
	    ...
	    $status = true;
            ...
            if ($unique_id) {
                ...
                $update_sql = "UPDATE $table SET district_name = ?, is_active = ? WHERE unique_id = ?";
                $stmt = $mysqli->prepare($update_sql);
                $stmt->bind_param("sss", $district_name, $is_active, $unique_id);
            } else {
                $columns = [
                    "district_name" => $district_name,
                    "is_active" => $is_active,
                ...
                $insert_sql = "INSERT INTO $table (district_name, is_active, unique_id) VALUES (?, ?, ?)";
                $stmt = $mysqli->prepare($insert_sql);
                $stmt->bind_param("sss", $district_name, $is_active, $columns['unique_id']);
            }
