...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        // Check if the count for the given recNo and hostel_id is 0
        $checkQuery = "
            SELECT COUNT(recNo) AS count 
            FROM attendancerecordinfo 
            WHERE recNo = :recNo AND hostel_id = :hostel_id
        ";
        $stmt = $pdo->prepare($checkQuery);
        $stmt->execute([':recNo' => $recNo, ':hostel_id' => $hostel_id]);
