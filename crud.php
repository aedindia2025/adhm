...
header("Access-Control-Allow-Origin: https://nallosaims.tn.gov.in");
...
switch ($action) {
    ...
    case 'updateStdDetails':
        ...
        $old_s1_unique_id = $_POST["s1_unique_id"];
        $app_type = base64_decode($_POST["app_type"]);
    ...
    case 'createupdate':
        ...
        if ($count > 0) {
            ...
            if ($unique_id) {
                ...
                $sql = "INSERT INTO $table (std_app_no, academic_year, application_type, uuid, student_type, entry_date, unique_id, std_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    case 'a_create':
        ...
        $s1_unique_id = base64_decode($encoded_s1_unique_id);
        $uuid = base64_decode($encoded_uuid);
        $dob = base64_decode($encoded_dob);
        $gender = base64_decode($encoded_gender);
        $name = base64_decode($encoded_name);
        $pc = base64_decode($encoded_pc);
        $fatherName = base64_decode($encoded_fatherName);
        $address = base64_decode($encoded_address);
        $addressLatLong = base64_decode($encoded_addressLatLong);
        $aadhar_no = base256_decode($encoded_aadhar_no);
    ...
    case 'a_ref_create':
        ...
        $sql = "SELECT COUNT(unique_id) AS count FROM $table_a_ref WHERE uuid = ? and s1_unique_id = ?";
        $params = [$uuid, $s1_unique_id];
    ...
    case 'aadhar_confirmation_add':
        ...
        if ($count > 0) {
            // Update existing record
            $sql_update = "UPDATE $table_s2 SET std_name=?, dob=?, age=?, gender=?, father_name=?, address=?, mobile_no=?, std_app_no=? WHERE s1_unique_id=?";
            $params_update = [
                $std_name,
                $std_dob,
                $std_age,
            ...
            // Insert new record
            $sql_insert = "INSERT INTO $table_s2 (std_name, dob, age, gender, father_name, address, mobile_no, std_app_no, s1_unique_id, entry_date, unique_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $params_insert = [
                $std_name,
                $std_dob,
                $std_age,
    ...
    case 'umis_no_check':
        ...
        // Prepare the SQL statement
        $sql = "SELECT s1_unique_id FROM $table WHERE is_delete = ? AND umis_no = ? order by id desc limit 1";
        $stmt = $mysqli->prepare($sql);
        $stmt->bind_param("is", $is_delete, $umis_no);
        $stmt->execute();
        $stmt->bind_result($s1_unique_id);
        $stmt->fetch();
        $stmt->close();
       
        if ($s1_unique_id) {
            $sql_fetch = "select status,renewal_status from std_app_s where unique_id = '$s1_unique_id' and is_delete = 0";
            $stmt_fetch = $mysqli->prepare($sql_fetch);
            $stmt_fetch->execute();
            $stmt_fetch->bind_result($app_status, $renewal_status);
            $stmt_fetch->fetch();
            $stmt_fetch->close();
            $status = true;
           
            if ($renewal_status != 0 && $renewal_status != 3) {
                $sql_fetch = "select exit_status from renewal where s1_unique_id = '$s1_unique_id'";
                $stmt_fetch = $mysqli->prepare($sql_fetch);
                $stmt_fetch->execute();
                $stmt_fetch->bind_result($exit_status);
                $stmt_fetch->fetch();
                $stmt_fetch->close();
    ...
    case 'emis_no_check':
        ...
        // Prepare the SQL statement
        $sql = "SELECT s1_unique_id FROM $table WHERE is_delete = ? AND emis_no = ? order by id desc limit 1";
        $stmt = $mysqli->prepare($sql);
        $stmt->bind_param("is", $is_delete, $emis_no);
        $stmt->execute();
        $stmt->bind_result($s1_unique_id);
    ...
    case 'get_degree':
        ...
        $hostel_district_options = course_name_master("", $no_umis_stream);
        // $hostel_district_options = institute_name("", $no_umis_stream);
    ...
    case 'hostel_sub_datatable':
        ...
        if ($result->status) {
            ...
            foreach ($res_array as $key => $value) {
                ...
                $value['s_no'] = $s_no++;
                $value['hostel_name'] = disname($value['hostel_name']) . ' - ' . $value['hostel_id'] . ' <br> <b style="color:blue">' . $hybrid_hostel . '</b> ';
                // $value['is_active'] = is_active_show($value['is_active']);
    ...
    case 'calculate_distances':
        ...
        if ($student_type == '65f00a259436412348') {
            ...
            $q2 = "SELECT umis_no,no_umis_inst_id FROM std_app_umis_s4 WHERE s1_unique_id = '$s1_unique_id'";
            $res2 = mysqli_query($conn, $q2);
            $umis = mysqli_fetch_assoc($res2);

            if ($umis['no_umis_inst_id']) {
                $q3 = "SELECT latitude, longitude FROM clg_lat_long WHERE instituteId = '" . $umis["no_umis_inst_id"] . "'";
