...
$checkbox_reason_options = checkbox_reason();
$checkbox_reason_options = select_option($checkbox_reason_options, "Select Reason", "");
...
switch ($action) {
    ...
    case 'auto_datatable':
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                // Convert name_diff to descriptive and colored label with checkbox logic
                ...
                if ($score == 1) {
                    ...
                    $checkbox = '<input type="checkbox" onchange="handleCheckboxChange(this,\'' . $row['std_reg_no'] . '\')" data-reg_no="' . $row['std_reg_no'] . '" style="margin-right:5px;">';
                ...
                $row['name_diff'] = $checkbox . $name_status;


                $selectBoxInst = '<select class="reason-select select2 form-control" data-type="inst" data-reg_no="' . $row['std_reg_no'] . '" style="display: none; margin-top: 5px;" >' . $checkbox_reason_options . '</select>';

                $selectBoxHostel = '<select class="reason-select select2 form-control" data-type="hostel" data-reg_no="' . $row['std_reg_no'] . '" style="display: none; margin-top: 5px;" >' . $checkbox_reason_options . '</select>';

                // Institute distance
                if (is_null($row['std_to_inst_distance'])) {
                    $row['std_to_inst_distance'] = '-';
                } else {
                    $inst_distance_check = floor($row['std_to_inst_distance']);
                    if ($inst_distance_check <= 5) {
                        $checkbox_inst = '<input type="checkbox" onchange="handleCheckboxChange(this,\'' . $row['std_reg_no'] . '\')" data-reg_no="' . $row['std_reg_no'] . '" data-type="inst" style="margin-right:5px;">';
                        $row['std_to_inst_distance'] = $checkbox_inst . '<span style="color: red; font-weight: bold;">' . $inst_distance_check . ' km</span><br>' . $selectBoxInst . '';
                    } else {
                ...
                // Hostel distance
                if (is_null($row['std_to_hostel_distance'])) {
                    $row['std_to_hostel_distance'] = '-';
                } else {
                    $host_distance_check = floor($row['std_to_hostel_distance']);
                    if ($host_distance_check <= 5 && $inst_distance_check <= 5) {
                        $checkbox_hostel = '<input type="checkbox" onchange="handleCheckboxChange(this,\'' . $row['std_reg_no'] . '\')" data-reg_no="' . $row['std_reg_no'] . '" data-type="hostel" style="margin-right:5px;">';
                        $row['std_to_hostel_distance'] = $checkbox_hostel . '<span style="color: red; font-weight: bold;">' . $host_distance_check . ' km</span>';
                    } elseif ($host_distance_check <= 5 && $inst_distance_check > 5) {
                        $checkbox_hostel = '<input type="checkbox" onchange="handleCheckboxChange(this,\'' . $row['std_reg_no'] . '\')" data-reg_no="' . $row['std_reg_no'] . '" data-type="hostel" style="margin-right:5px;">';
                        $row['std_to_hostel_distance'] = $checkbox_hostel . '<span style="color: red; font-weight: bold;">' . $host_distance_check . ' km</span><br>' . $selectBoxHostel . '';
                    } else {
                        $row['std_to_hostel_distance'] = '<span style="color: green; font-weight: bold;">' . $host_distance_check . ' km</span>';
                    }
                }

                $has_checkbox = ($checkbox !== '' || $checkbox_inst !== '' || $checkbox_hostel !== '');

                $buttons_html = '
                <div class="action-buttons" data-reg_no="' . $row['std_reg_no'] . '">
                    <button class="transferbtn" ' . ($has_checkbox ? 'disabled' : '') . ' data-std_name="' . $row['std_name'] . '" data-reg_no="' . $row['std_reg_no'] . '" data-s1_unique_id="' . $row['s1_unique_id'] . '" onclick="transferStudent(this)">Transfer</button>
                    <button class="exitBtn" ' . ($has_checkbox ? 'disabled' : '') . ' data-std_name="' . $row['std_name'] . '" data-reg_no="' . $row['std_reg_no'] . '" data-s1_unique_id="' . $row['s1_unique_id'] . '" >Exit Hostel</button>
                    <button class="renewBtn" ' . ($has_checkbox ? 'disabled' : '') . ' data-std_name="' . $row['std_name'] . '" data-reg_no="' . $row['std_reg_no'] . '" data-s1_unique_id="' . $row['s1_unique_id'] . '" data-inst_dist = "' . $inst_distance_check . '" data-host_dist = "' . $host_distance_check . '" onclick="autoRenewal(this)">Renew</button>
                </div>';

                $row['action'] = $buttons_html;
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                if ($score == 1) {
                    ...
                    $checkbox = '<input type="checkbox" onchange="m_handleCheckboxChange(this,\'' . $row['std_reg_no'] . '\')" data-reg_no="' . $row['std_reg_no'] . '" style="margin-right:5px;">';
                    $name_status = '<span style="color: orange; font-weight: bold;">Partially Matched</span>';
                } else {
                    $checkbox = '<input type="checkbox" onchange="m_handleCheckboxChange(this,\'' . $row['std_reg_no'] . '\')" data-reg_no="' . $row['std_reg_no'] . '" style="margin-right:5px;">';
                    $name_status = '<span style="color: red; font-weight: bold;">Mismatched</span>';
                ...
                $selectBoxInst = '<select class="m_reason-select select2 form-control" data-type="m_inst" data-reg_no="' . $row['std_reg_no'] . '" style="display: none; margin-top: 5px;">
                                        ' . $checkbox_reason_options . '
                                  </select>';

                $selectBoxHostel = '<select class="m_reason-select select2 form-control" data-type="m_hostel" data-reg_no="' . $row['std_reg_no'] . '" style="display: none; margin-top: 5px;">
                    ' . $checkbox_reason_options . '
                </select>';
                ...
                if (is_null($row['std_to_inst_distance'])) {
                    ...
                    $inst_distance_check = floor($row['std_to_inst_distance']);
                    if ($inst_distance_check <= 5) {
                        $checkbox_inst = '<input type="checkbox" onchange="m_handleCheckboxChange(this,\'' . $row['std_reg_no'] . '\')" data-reg_no="' . $row['std_reg_no'] . '" data-type="m_inst" style="margin-right:5px;">';
                        $row['std_to_inst_distance'] = $checkbox_inst . '<span style="color: red; font-weight: bold;">' . $inst_distance_check . ' km</span><br>' . $selectBoxInst . '';
                    } else {
                        $row['std_to_inst_distance'] = '<span style="color: green; font-weight: bold;">' . $inst_distance_check . ' km</span>';
                    }
                ...
                if (is_null($row['std_to_hostel_distance'])) {
                    ...
                    $host_distance_check = floor($row['std_to_hostel_distance']);
                    if ($host_distance_check <= 5 && $inst_distance_check <= 5) {
                        $checkbox_hostel = '<input type="checkbox" onchange="m_handleCheckboxChange(this,\'' . $row['std_reg_no'] . '\')" data-reg_no="' . $row['std_reg_no'] . '" data-type="m_inst" style="margin-right:5px;">';
                        $row['std_to_hostel_distance'] = $checkbox_hostel . '<span style="color: red; font-weight: bold;">' . $host_distance_check . ' km</span>';
                    } else if ($host_distance_check <= 5 && $inst_distance_check > 5) {
                        $checkbox_hostel = '<input type="checkbox" onchange="m_handleCheckboxChange(this,\'' . $row['std_reg_no'] . '\')" data-reg_no="' . $row['std_reg_no'] . '" data-type="m_inst" style="margin-right:5px;">';
                        $row['std_to_hostel_distance'] = $checkbox_hostel . '<span style="color: red; font-weight: bold;">' . $host_distance_check . ' km</span><br>' . $selectBoxHostel . '';
                    } else {
                        $row['std_to_hostel_distance'] = '<span style="color: green; font-weight: bold;">' . $host_distance_check . ' km</span>';
                ...
                $buttons_html = '
                <div class="action-buttons" data-reg_no="' . $row['std_reg_no'] . '">
                    <button class="m_transferbtn" ' . ($has_checkbox ? 'disabled' : '') . ' data-std_name="' . $row['std_name'] . '" data-reg_no="' . $row['std_reg_no'] . '" data-s1_unique_id="' . $row['s1_unique_id'] . '" onclick="m_transferStudent(this)">Transfer</button>
                    <button class="m_exitBtn" ' . ($has_checkbox ? 'disabled' : '') . ' data-std_name="' . $row['std_name'] . '" data-reg_no="' . $row['std_reg_no'] . '" data-s1_unique_id="' . $row['s1_unique_id'] . '" >Exit Hostel</button>
                    <button class="m_renewBtn" ' . ($has_checkbox ? 'disabled' : '') . ' data-std_name="' . $row['std_name'] . '" data-reg_no="' . $row['std_reg_no'] . '" data-m_inst_dist = "' . $inst_distance_check . '" data-m_host_dist = "' . $host_distance_check . '" data-s1_unique_id="' . $row['s1_unique_id'] . '" onclick="manualRenewal(this)">Request Renew</button>
                </div>';
        ...
        if ($action_obj_h->status && !empty($action_obj_h->data)) {
            ...
            if (!empty($transfer_hostel)) {
                ...
                if ($hostel_info->status && !empty($hostel_info->data)) {
                    ...
                    $update_std_s7_where = 's1_unique_id = "' . $common_unique_id . '"';
    ...
    case 'manualRenewal':
        ...
        $update_columns = ["manual_submit_status" => 1, "dist_reason" => $reasons];
    ...
    case 'updateDistance':
        ...
        function getSingleRow($pdo, $table, $columns, $where)
        {
            $result = $pdo->select([$table, $columns], $where);
            return isset($result->data[0]) ? $result->data[0] : null;
        }

        // 1. Get student lat/lng
        $stuData = getSingleRow($pdo, "aadhar", ["latitude", "longitude"], 's1_unique_id = "' . $s1_unique_id . '"');
        $stuLat = $stuData['latitude'] ?? null;
        $stuLong = $stuData['longitude'] ?? null;

        // 2. Get hostel ID
        $hostelData = getSingleRow($pdo, "std_app_s", ["hostel_1"], 'unique_id = "' . $s1_unique_id . '"');
        $hostel1 = $hostelData['hostel_1'] ?? null;

        // 3. Get hostel lat/lng
        $hostelLoc = getSingleRow($pdo, "hostel_name", ["latitude", "longitude"], 'unique_id = "' . $hostel1 . '"');
        $hostelLat = $hostelLoc['latitude'] ?? null;
        $hostelLong = $hostelLoc['longitude'] ?? null;

        // 4. Get institute lat/lng
        $instLoc = getSingleRow($pdo, "clg_lat_long", ["latitude", "longitude"], 'instituteId = "' . $instituteId . '"');
        $instLat = $instLoc['latitude'] ?? null;
        $instLong = $instLoc['longitude'] ?? null;
        ...
        // 6. Update `renewal` table
        $updateRenewalData = [
            "std_to_inst_distance" => $stdToInst ?? NULL,
            "std_to_hostel_distance" => $stdToHostel ?? NULL
        ];

        $updateRenewal = $pdo->update("renewal", $updateRenewalData, 's1_unique_id = "' . $s1_unique_id . '"');

        // 7. Update `std_app_s` table
        $updateStdAppSData = [
            "std_to_inst_distance" => $stdToInst ?? NULL,
            "std_to_hostel_distance" => $stdToHostel ?? NULL
        ];

        $updateStdAppS = $pdo->update("std_app_s", $updateStdAppSData, 'unique_id = "' . $s1_unique_id . '"');

        // 8. Response
        if ($updateRenewal && $updateStdAppS) {
            echo json_encode([
                "status" => true,
                "message" => "Distance calculated successfully.",
                "inst_distance" => $stdToInst,
                "hostel_distance" => $stdToHostel
            ]);
        } else {
            echo json_encode([
                "status" => false,
                "message" => "Distance update failed."
            ]);
        }
