...
function sub_list_datatable1(table_id = "", form_name = "", action = "") {
	...
	var datatable = table.DataTable({
		...
		buttons: [		
		{
			...
			title: 'Print For Dispatch',
			filename: 'print_for_dispatch'
		}
	],
	});
}
...
function init_datatable(table_id = '', form_name = '', action = '', filter_data = '') {
	...
	lengthMenu: [
		...
		"columnDefs": [
			...
			{ "targets": [0, 3, 4, 5], "className": "dt-right" },
...
function handleCheckbox(checkbox) {
    ...
    const s1_unique_id = checkbox.getAttribute("data-s1_unique_id");

    // Find the matching select box
    const selectBox = document.querySelector(`select.reason-select[data-type="${type}"][data-s1_unique_id="${s1_unique_id}"]`);

    if (checkbox.checked) {
        // Show the select box if checkbox is checked
        selectBox.style.display = "block";
    } else {
        // Hide and clear the select box if unchecked
