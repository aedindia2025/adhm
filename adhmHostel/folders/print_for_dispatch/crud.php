...
switch ($action) {
    ...
    case 'datatable':
        ...
        $param_types = "ss";
        $bind_values = [$is_delete, $hostel_id];
    ...
    case 'dispatch_datatable':
        ...
        $columns = [
            ...
            "academic_year",
            "(SELECT renewal_umis_no FROM std_app_umis_s4 where std_app_umis_s4.s1_unique_id = std_app_s.unique_id) as renewal_umis_no",
            "application_type"
        ];
        $table1 = 'std_app_s';
        ...
        // echo  $is_delete;
        // echo $_POST["appilicationtype"];
        $where = "is_delete = ? AND hostel_1 = ? AND (batch_no IS NULL or batch_no = '')";
        $params = [];
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                $jac_similarity = jaccardSimilarity($row['std_name'], $row['emis_umis_name']);

                // Convert name_diff to descriptive and colored label with checkbox logic
                $score = floatval($jac_similarity);
                $checkbox = '';

                // Define checkbox eligibility
                $showCheckbox = ($row['application_type'] == 1 || !is_null($row['renewal_umis_no']));

                if ($score == 1) {
                    $name_status = '<span style="color: green; font-weight: bold;">Matched</span>';
                } elseif ($score >= 0.6) {
                    if ($showCheckbox) {
                        $checkbox = '<input type="checkbox" id="name_check_' . $row['s1_unique_id'] . '" style="margin-right:5px;">';
                    }
                ...
                $row['unique_id'] = '<input class="myCheck" type="checkbox" id="invoice_check" name="invoice_check[]" disabled checked>
                ...
                $selectBoxInst = '<select class="reason-select select2 form-control" data-type="inst" data-s1_unique_id="' . $row['s1_unique_id'] . '" style="display: none; margin-top: 5px;" >' . $checkbox_reason_options . '</select>';

                $selectBoxHostel = '<select class="reason-select select2 form-control" data-type="hostel" data-s1_unique_id="' . $row['s1_unique_id'] . '" style="display: none; margin-top: 5px;" >' . $checkbox_reason_options . '</select>';
                ...
                if (is_null($row['std_to_inst_distance'])) {
                    ...
                    if ($inst_distance_check <= 5) {
                        $checkbox_inst = '';
                        if ($showCheckbox) {
                            $checkbox_inst = '<input type="checkbox" id="inst_check_' . $row['s1_unique_id'] . '" data-type="inst" data-s1_unique_id="' . $row['s1_unique_id'] . '" onchange="handleCheckbox(this)"  style="margin-right:5px;">';
                        }
                ...
                if (is_null($row['std_to_hostel_distance'])) {
                    ...
                    if ($host_distance_check <= 5 && $inst_distance_check <= 5) {
                        $checkbox_hostel = '';
                        if ($showCheckbox) {
                            $checkbox_hostel = '<input type="checkbox" id="hostel_check_' . $row['s1_unique_id'] . '" data-type="hostel" data-s1_unique_id="' . $row['s1_unique_id'] . '" onchange="handleCheckbox(this)" style="margin-right:5px;">';
                        }
    ...
    case 'batch_add':
        ...
        $sql = $mysqli->prepare("select count(id) as cnt from batch_creation where batch_no = '" . $batch_no . "' and std_app_no = '" . $std_app_no . "' and s1_unique_id = '" . $s1_unique_id . "'");
        $sql->execute();
        ...
        if ($row['cnt'] > '0') {
            ...
            unique_id = ?,
            dist_reason = ?
        WHERE std_app_no = ? AND batch_no = ? and s1_unique_id = ?";
            ...
            $insert_sql = "INSERT INTO batch_creation 
                    (s1_unique_id, applied_date, hostel_name, hostel_taluk, hostel_district, std_name, std_app_no, academic_year, batch_cr_date, batch_no, unique_id, dist_reason) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        $update_sql = "UPDATE std_app_s 
                    SET batch_cr_date = ?, batch_no = ?, dist_reason = ? 
                    WHERE is_delete = '0' AND unique_id = ?";

        $update_stmt = $mysqli->prepare($update_sql);
        $update_stmt->bind_param("ssss", date('Y-m-d'), $batch_no, $reason, $s1_unique_id);
        $update_result = $update_stmt->execute();
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                if (!empty($std_emis_name['emis_name'])) {
                    $row['std_umis_emis_name'] = $std_emis_name['emis_name'];
                    $row['std_umis_emis_no'] = $std_emis_name['emis_no'];
                    $row['std_umis_emis_ins_name'] = $std_emis_name['school_name'];
                    $row['std_umis_emis_cls_deg'] = $std_emis_name['class'];
                } elseif (!empty($umis_name['umis_name'])) {
                    $row['std_umis_emis_name'] = $umis_name['umis_name'];
                    $row['std_umis_emis_no'] = $umis_name['umis_no'];
                    $row['std_umis_emis_ins_name'] = $umis_name['umis_clg_name'];
                    $row['std_umis_emis_cls_deg'] = $umis_name['umis_std_degree'];
                } elseif (!empty($no_umis_name['no_umis_name'])) {
                    $row['std_umis_emis_name'] = $no_umis_name['no_umis_name'];
                    $row['std_umis_emis_no'] = 'No UMIS Entry';
                    $row['std_umis_emis_ins_name'] = $no_umis_name['no_umis_clg'];
                    $row['std_umis_emis_cls_deg'] = courseName($no_umis_name['no_umis_course']);
                } else {
                    $row['std_umis_emis_name'] = '-'; // Default value if none found
                    $row['std_umis_emis_no'] = '-'; // Default value if none found
                    $row['std_umis_emis_ins_name'] = '-'; // Default value if none found
                    $row['std_umis_emis_cls_deg'] = '-'; // Default value if none found
                }

                $jac_similarity = jaccardSimilarity($row['std_name'], $row['std_umis_emis_name']);
