...
switch ($action) {
    ...
    case 'datatable':
        ...
        $table_details = [
            $table2 . " , (SELECT @a:= " . $start . ") AS a ",
            $columns
        ];
        $where = "is_delete = 0 and hostel_name = '" . $_SESSION['hostel_id'] . "' group by entry_date";
        $order_by = "";
        ...
        if ($result->status) {
            ...
            foreach ($res_array as $key => $value) {
                ...
                $value['s_no'] = $sno + 1;
                $value['total_cnt'] = total_count();
                $value['present'] = present_count($value['entry_date']);
                $value['absent'] = $value['total_cnt'] - $value['present'];
    ...
    case 'att_cr_datatable':
        ...
        $where = "is_delete = '0' and hostel_1 = '" . $_SESSION['hostel_id'] . "'";
        ...
        if ($result->status) {
            ...
            foreach ($res_array as $key => $value) {
                ...
                $value['unique_id'] = '<input class="myCheck" type="checkbox" id="invoice_check" name="invoice_check[]" checked>
                    <input type="hidden" id="s1_unique_id" name="s1_unique_id[]" value="' . $value['unique_id'] . '">  
                    ...
                    <input type="hidden" id="std_name" name="std_name[]" value="' . $value['std_name'] . '">
                    <input type="hidden" id="academic_year" name="academic_year[]" value="' . $value['academic_year'] . '">
                    <input type="hidden" id="std_reg_no" name="std_reg_no[]" value="' . $value['std_reg_no'] . '">';
    ...
    case 'att_create':
        ...
        $select_where = 'entry_date = "' . $cur_date . '"  AND is_delete = 0 and s1_unique_id = "' . $_POST["s1_unique_id"] . '"  group by entry_date';
        ...
        if ($data[0]["count"]) {
            ...
            $action_obj = $pdo->insert($table2, $columns);
            $msg = "create";
