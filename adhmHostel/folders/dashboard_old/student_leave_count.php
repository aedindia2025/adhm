    <!-- <link href="../assets/css/bootstrap.min.css" rel="stylesheet" type="text/css" id="bs-default-stylesheet" />
    <link href="../assets/css/app.min.css" rel="stylesheet" type="text/css" id="app-default-stylesheet" /> -->
    <style>

body {
    
    font-family: 'Poppins', sans-serif;
}
    	body {
    		background-color: #fff;
    	}

    	.zone_recom {
    		border: 1px solid #ccc;
    		padding: 14px;
    		margin-bottom: 30px;
    	}

    	.box1 h3 {
    		background-color: #f0f0f0;
    		padding: 4px;
    		text-align: center;
    		font-weight: 700;
    		color: #333;
    		font-size: 14px;
    	}

    	.bd-highlight {
    		font-size: 14px;
    		color: #444;
    	}

    	.contn_info.d-flex h6 {
    		text-align: right;
    		font-size: 11.5px;
    		margin-bottom: 4px;
    	}

    	.contn_info.d-flex h5 {
    		color: #000;
    		font-size: 11.5px;
    		margin-bottom: 4px;
    	}

    	.contn_info.d-flex p {
    		margin-bottom: 4px;
    	}

    	.zone_boxbor {

    		margin-bottom: 20px;

    	}

    	.zone_recom1,
    	.zone_recom3 {
    		/* border: 1px solid #ccc; */
    		padding: 4px;
    	}

    	.zone_recom2 {
    		/* border: 1px solid #ccc; */
    		padding: 4px;
    	}

    	.col-md-4 {
    		width: 33.33333333%;
    		padding-left: 5px;
    		padding-right: 5px;
    	}

    	.col-md-8 {
    		flex: 0 0 auto;
    		width: 66.66666667%;
    	}

    	.col-md-4.wid1 {
    		width: 45%;
    	}

    	.col-md-8.wid2 {
    		width: 55%;
    	}

    	table,
    	th,
    	td {
    		border: 1px solid #ccc;
    		border-collapse: collapse;
			
    	}

    	th,
    	td {
    		padding: 5px;
    		text-align: left;
			
    	}

    	.print_icon {
    		text-align: right;
    		font-size: 33px;
    	}
    </style>
    <?php

	include '../../config/dbconfig.php';

	



	?>
<!-- <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous"> -->

    <div class="container-fluid" style="background-color:#fff;">
    	<div class="compl_print pt-2">
    		<div class="zone_boxbor">
    			<div class="row">

    				<div class="col-md-12">

    					<div class="clearfix">
							<center>
                                            <div class=" mb-3 mt-1 text-center vendorListHeading2" >
                                                <img src="../../../assets/images/ad-logo.png" alt="dark logo" height="50">
                                            </div>
	</center>

											<!-- <div class="col-sm-12 mb-2">
                                                <div class=" mt-1 vendorListHeading">
                                                    <p><b>Hostel Information</b></p>
													
                                                </div> -->
                                            <!-- </div>end col -->
											<div class="col-sm-12 ">
                                                <div class="mt-0 float-sm-left">
												<div class="row">
                                                <center>
                                                    <h3>STUDENT LEAVE COUNT</h3></center>
													<div class="col-sm-12">
													<p class="font-12">Hostel District   :  &nbsp;<strong><?=$_SESSION["district_name"];?></strong></p>
													</div>

													<div class="col-sm-12">
													<!-- <p class="font-12">Date   :  &nbsp;<strong><?=date('d-m-Y');?></strong></p> -->
													</div>
													
													
                                                </div>
                                            </div><!-- end col -->
											</div>
    					<div class="zone_recom3">
    						<div class="box1">
    							<table cellspacing="0" cellpadding="0" class="" width="100%" style="font-family: monospace;">
    								<thead class="colspanHead">
    									<tr>
    										<th width="5%" colspan="1" class="blankCell">S.No</th>
    										<th width="10%" colspan="1" class="blankCell">Student Name</th>
    										<th width="10%" colspan="1" class="blankCell">Student Reg No</th>
                                            <th width="10%" colspan="1" class="blankCell">Hostel Name</th>
    										<th width="10%" colspan="1" class="blankCell">Reason</th>
                                            <th width="10%" colspan="1" class="blankCell">Approval Status</th>
                                            <th width="10%" colspan="1" class="blankCell">No of Days</th>
                                            <th width="10%" colspan="1" class="blankCell">Taluk Name</th>
    										
    										

    									</tr>
    								</thead>
    								<?php
									$start = 0;
									
                                    $table = "leave_application";
                                   
                                    $hostel_name = $_SESSION["hostel_id"]; 

									$today  =  date('Y-m-d');


									$columns_list    = [
										"@a:=@a+1 s_no",
                                        "student_name",
                                        "student_id",
                                        "(select hostel_name from hostel_name where unique_id= $table.hostel_name)as hostel_name",
                                        "reason",
                                        "approval_status",
                                        "no_of_days",
                                        "(select taluk_name from taluk_creation where unique_id = $table.taluk_name)as taluk_name"
									];

                                    $where_list = "is_delete = 0  AND hostel_name = '".$hostel_name."' ";

									$table_details_list  = [
										$table . ", (SELECT @a:= " . $start . ") AS a ",
										$columns_list
									];



									$result_leave    = $pdo->select($table_details_list,$where_list);

									// print_r($result_leave);


									if ($result_leave->status) {

										$res_array      = $result_leave->data;

										$table_data     = "";
										if (count($res_array) == 0) {

											$table_data .= "<tr>";

											$table_data .= "<td colspan=9; style='text-align:center'>NO DATA FOUND</td>";
											$table_data .= "</tr>";
										} else {
											foreach ($res_array as $key => $value) {
												// echo "dd";

                                                $staff_name = $value['student_name'];

                                                $designation  = $value['student_id'];

                                                $hostel_name  = $value['hostel_name'];

                                                $reason  = $value['reason'];

                                                $approval_status  = $value['approval_status'];

                                                $no_of_days  = $value['no_of_days'];

                                                $taluk_name  = $value['taluk_name'];

                                                if($value['approval_status']=='1'){
                                                    $value['approval_status']='Pending';
                                                }
                                                else if($value['approval_status']=='2'){
                                                    $value['approval_status']='Approved';
                                                }
                                                else if($value['approval_status']=='3'){
                                                    $value['approval_status']='Rejected';
                                                }
                                             
												
												// $value['entry_date']        = disdate($value['entry_date']);

												$table_data .= "<tr>";

												$table_data .= "<td>" . $value['s_no'] . "</td>";
												$table_data .= "<td style = 'text-align : left'>" .$staff_name ."</td>";
												$table_data .= "<td style = 'text-align : left'>" .$designation ."</td>";
												$table_data .= "<td style = 'text-align : left'>" . $hostel_name . "</td>";
                                                $table_data .= "<td style = 'text-align : left'>" . $reason. "</td>";
                                                $table_data .= "<td style = 'text-align : left'>" .  $value['approval_status']. "</td>";
                                                $table_data .= "<td style = 'text-align : left'>" .   $no_of_days  . "</td>";
                                                $table_data .= "<td style = 'text-align : left'>" . $taluk_name . "</td>";
                                               
												
												$table_data .= "</tr>";
											}
										}
									
                                    
									}
									?>

    								<tbody>
    									<?php echo $table_data; ?>
    								</tbody>
    							</table>
    						</div>
    					</div>
    				</div>


    			</div>
    		</div>


    	</div>
    </div>