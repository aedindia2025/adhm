    						...
    						<div class="box1">
    							<table cellspacing="0" cellpadding="0" class="" width="100%" style="font-family: monospace;">
									...
									if ($_GET['status'] == "approved") {
										$where_list = " is_delete = 0 and status = '1' and hostel_1 = '".$_SESSION['hostel_id']."'";
									}
									if ($_GET['status'] == "rejected") {
										$where_list = " is_delete = 0 and status = '2' and hostel_1 = '".$_SESSION['hostel_id']."'";
									}
									if ($_GET['status'] == "acceptance") {
										$where_list = " is_delete = 0 and batch_no != '' and hostel_name = '".$_SESSION['hostel_id']."'";
									}
									if ($_GET['status'] == "applied") {
										$where_list = " is_delete = 0 and hostel_1 = '".$_SESSION['hostel_id']."'";
									}
									...
									if($_GET['status'] == "acceptance"){
										$columns_list    = [
											...
											"std_app_no",
											"(select aname from aadhar where aadhar.s1_unique_id = batch_creation.s1_unique_id) as std_name",
											"(select age from std_app_s2 where std_app_s2.s1_unique_id = batch_creation.s1_unique_id) as age",
											"(select com_name from std_app_s5 where batch_creation.s1_unique_id = std_app_s5.s1_unique_id) as com_name",
											"status",
											// "entry_date",
											...
											$columns_list    = [
												...
												"(select com_name from std_app_s5 where std_app_s.unique_id = std_app_s5.s1_unique_id) as com_name",
												"status",
												// "entry_date",
									...
									if ($result->status) {
										...
										if (count($res_array) == 0) {
											...
											foreach ($res_array as $key => $value) {
												...
												$table_data .= "<td style = 'text-align : left'>" . $value['com_name'] . "</td>";
