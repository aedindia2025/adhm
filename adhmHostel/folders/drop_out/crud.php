...
switch ($action) {
    ...
    case 'createupdate':
        ...
        $vali_dropout_reason = filter_input(INPUT_POST, 'dropout_reason', FILTER_SANITIZE_STRING);
        $vali_cust_reason = filter_input(INPUT_POST, 'cust_reason', FILTER_SANITIZE_STRING);
        ...
        if (
            !$vali_student_id || !$vali_student_name || !$vali_drop_discontinue_date || !$vali_dropout_reason
            ...
            $cust_reason = sanitizeInput($_POST["cust_reason"]);
            ...
            $select_query = 'SELECT COUNT(unique_id) AS count FROM ' . $table . ' WHERE student_id = ? AND is_delete = ? and status = "1"';
            ...
            if ($data["count"] > 0) {
                ...
                if ($unique_id) {
                    ...
                    $update_query = 'UPDATE ' . $table . ' SET student_id = ?, student_name = ?, drop_discontinue_date = ?, dropout_reason = ?, cust_reason = ?, staff_id = ?, staff_name = ?, district_id = ?, taluk_id = ?, hostel_id = ?, acc_year = ? WHERE unique_id = ?';
