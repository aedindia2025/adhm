...
function stock_entry_delete(unique_id = "") {
	...
	function stock_sub_add_update() {
		...
		// var qty = $("#qty").val();
		// var month_indent = $("#month_indent").val();
		// alert(amount);
		var unit = $("#unit").val();
	
		
 var qty = parseFloat($("#qty").val()) || 0;          // convert to number
    var month_indent = parseFloat($("#month_indent").val()) || 0; //
		 if (qty > month_indent) {
       swal.fire({
    icon: 'warning',       
    title: 'Warning!',   
    text: 'Not allowed: quantity exceeds monthly indent!',
    confirmButtonText: 'OK' 
		...
		if(!purchase_item || !item_name || !qty || !rate){
	...
	function stock_sub_datatable(table_id='',form_name='',action='') {
		...
		"columnDefs": [
			...
			{ "targets": [0,3,5,6], "className": "dt-right" },
...
function print(file_name) {
	...
	function stock_sub_delete(val,unique_id) {

		//  var data 	 = val;
		var data = "id=" + val + "&unique_id=" + unique_id + " &action=sub_delete";
