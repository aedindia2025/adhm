...
switch ($action) {
    case 'main_createupdate':
        ...
        if ($data["count"] > 0) {
            ...
            if ($unique_id) {
                $sql = "UPDATE $table_main SET entry_date = ?, to_date = ?, hostel_name = ?, stock_id = ?, district = ?, taluk = ?, screen_unique_id = ? WHERE unique_id = ?";
                $params = [$entry_date, $to_date, $hostel_name, $stock_id, $district, $taluk, $screen_unique_id, $unique_id];
                $types = "ssssssss";
            } else {
                $generated_unique_id = unique_id($prefix);
                $sql = "INSERT INTO $table_main (entry_date, to_date, hostel_name, stock_id, district, taluk, screen_unique_id, unique_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                $params = [$entry_date, $to_date, $hostel_name, $stock_id, $district, $taluk, $screen_unique_id, $generated_unique_id];
                $types = "ssssssss";
            }
    ...
    case 'overall_createupdate':
        ...
        $stmt = $mysqli->prepare("UPDATE $overall_table SET entry_date = ?, to_date = ?, hostel_unique_id = ?, academic_year = ?, district_unique_id = ?, taluk_unique_id = ? WHERE stock_id = ?");

        // Bind the parameters to the placeholders
        $stmt->bind_param("sssssss", $entry_date, $to_date, $hostel_name, $academic_year, $district, $taluk, $stock_id);
    ...
    case 'createupdate_overall':
        ...
        $params = [$purchase_item, $item_name, $parsed_qty, $screen_unique_id, $act_qty, $stock_id, $generated_unique_id];
        $types = "sssssss";
        //}
    ...
    case 'createupdate':
        ...
        // $floatValue1 = $actual_qty;
        // $act_qty = number_format($floatValue1, 3, '.', '');
        ...
        $params = [$purchase_item, $item_name, $actual_qty, $parsed_qty, $stock_id, $screen_unique_id, $generated_unique_id];
        $types = "sssssss";
    ...
    case 'stock_out_sub_datatable':
        ...
        $columns = [
            "' ' as sno",
            "(select item_category from item_category where unique_id = $table.purchase_item) as category",
            "(select item from item where unique_id = $table.item_name) as item_name",
            "actual_qty",
            "qty",
            "unique_id",
            "id",
    ...
    case 'product_supply':
        ...
        $product_type_option = item_type_name('', $purchase_item, $stock_in_val);
