...
function fetchLocation() {
	if (navigator.geolocation) {
		navigator.geolocation.getCurrentPosition(
			...
			function (error) {
				...
				switch (error.code) {
					case error.PERMISSION_DENIED:
						...
						var enableLocation = confirm(
							"Location access is required for this feature. Do you want to learn how to enable location services?"
						);
						if (enableLocation) {
							// Redirect to a guide for enabling location services
							window.open(
								"https://support.google.com/chrome/answer/142065?co=GENIE.Platform%3DDesktop&hl=en",
								"_blank"
							);
						}
						break;
					case error.POSITION_UNAVAILABLE:
						...
						break;
					case error.TIMEOUT:
						alert("The request to fetch location timed out.");
						break;
			...
			{
				enableHighAccuracy: true, // Use high-accuracy location (e.g., GPS)
...
function submit_sub(unique_id = "") {
	...
	var image_s = document.getElementById("go_attachment");
...
function openCameraWindow1() {
    window.open('folders/hostel_creation/camera.php', '_blank', 'width=500,height=600');
}

function openCameraWindow2() {
    window.open('folders/hostel_creation/camera1.php', '_blank', 'width=500,height=600');
}

function openCameraWindow3() {
    window.open('folders/hostel_creation/camera2.php', '_blank', 'width=500,height=600');
}
...
function hostel_update(unique_id = "") {
	...
	if ((file_val != '' || file_name != '') && special_tahsildar != '' && assembly_const != '' && parliment_const != '' && hostel_location != '' && yob != '' && distance_btw_phc != '' && phc_name != '' && distance_btw_ps != '' && ps_name != '' && staff_count != '' && entrance_image != '' && dining_image != '' && building_image != '') {
...
function print_view_image(file_name) {
	var iframeContent = '<html><head><style>body, html {width: 100%; height: 100%; margin: 0; padding: 0;}</style></head><body>' +
		'<iframe id="myIframe" src="uploads/hostel_creation' + file_name + '"' + ' style="height:100%; width:100%; border:none;"></iframe>' +
		'</body></html>';
...
function print_pdf_go(file_name) {
	var pdfUrl = "uploads/hostel_creation/" + file_name;
