...
function btn_delete_uniqueid($folder_name = "", $unique_id = "", $std_reg_no = "")
{

    $final_str = '<a href="#" onclick="' . $folder_name . '_delete(\'' . $unique_id . '\', \'' . $std_reg_no . '\')" class="font-18 text-danger" data-bs-toggle="tooltip" data-bs-placement="top" aria-label="Delete" data-bs-original-title="Delete"><i class="uil uil-trash"></i></a>';
    return $final_str;
}

function hostel_gender_type($unique_id = "", $taluk_name = "", $gender_type = "", $hostel_type = "")
{
...
function pruchase_category($unique_id = "")
{
    global $pdo;
...
function product_type_names($unique_id = "", $product_category = "", $ex_stk_val = "")
{
    // echo $zone_name;
// print_r($unique_id);die();
    ...
    if ($ex_stk_val) {
        // $where              = [];
        $where = "";
        $where = ' unique_id NOT IN (' . $ex_stk_val . ') and is_delete = "0" and product_category = "' . $product_category . '"';
    }
...
function btn_delete_stk($folder_name = "", $screenunique_id = "", $unique_id = "")
{

    $final_str = '<a href="#" onclick="' . $folder_name . '_delete(\'' . $screenunique_id . '\', \'' . $unique_id . '\')" class="font-18 text-danger" data-bs-toggle="tooltip" data-bs-placement="top" aria-label="Delete" data-bs-original-title="Delete"><i class="uil uil-trash"></i></a>';
...
function student_list($unique_id = '', $no_umis_stream = '')
{
    global $pdo;
    ...
    $table_columns = [
        "GROUP_CONCAT(CONCAT('\"', student_id, '\"') SEPARATOR ', ') AS student_id"
    ];
    ...
    $where = "status != '2' and is_delete = '0' and hostel_id = '" . $_SESSION['hostel_id'] . "'";

    $district_name_list = $pdo->select($table_details, $where);
    // print_r($district_name_list);
    if ($district_name_list->status) {
        return $district_name_list->data[0]['student_id'];
    } else {
        print_r($district_name_list);
...
function student_id($unique_id = "", $hostel_id = "", $student_list = "")
{
    ...
    $where = "is_delete = '0' and dropout_status != 2";

    if ($hostel_id) {
        $where .= " and hostel_1 = '" . $hostel_id . "'";
    }
    ...
    if ($student_list) {
        $where = [];
        $where = "";
        $where = ' unique_id NOT IN (' . $student_list . ') and is_delete = "0" and dropout_status != 2 and hostel_1 = "' . $hostel_id . '"';
        // $where = "";
    }
...
function digital_category($digital_category = "")
{
    // echo $zone_name;
    global $pdo;
    ...
    if ($digital_category) {
        // $where              = [];
        $where["unique_id"] .= $digital_category;
    }


    $digital_category_options = $pdo->select($table_details, $where);
    // print_r($facility_name_options);die();

    if ($digital_category_options->status) {
        return $digital_category_options->data;
    } else {
        print_r($digital_category_options);
        return 0;
    }
}
...
function kitchen_category_type($unique_id = "", $ex_stk_val = "")
{
    // echo $zone_name;
// print_r($unique_id);die();
    global $pdo;
    ...
    $category_type_name = $pdo->select($table_details, $where);

    // print_R( $product_type_list);
    if ($category_type_name->status) {
        return $category_type_name->data;
    } else {
        print_r($category_type_name);
        return 0;
    }
}


function kitchen_asset_type($unique_id = "", $ex_stk_val = "")
{
    // echo $zone_name;
// print_r($unique_id);die();
    global $pdo;
...
function asset_type($unique_id = "", $asset_type = "")
{
    // echo $zone_name;
    global $pdo;
    ...
    $aset_type_list = $pdo->select($table_details, $where);
    //  print_r($aset_type_list);

    if ($aset_type_list->status) {
        return $aset_type_list->data;
    } else {
        print_r($aset_type_list);
        return 0;
    }
...
function list_asset_type($unique_id = "", $asset_type = "", $list_type = "")
{
    // echo $zone_name;
    global $pdo;
    ...
    if ($list_type == '1') {
        ...
        $table_columns = [
            // "unique_id",
            "asset",
            "(select digital_asset from digital_asset where digital_asset.unique_id=view_moveables_asset.asset) as asset_name"
            // "unit"
        ];
    }
    ...
    $asset_type_list = $pdo->select($table_details, $where, "", "", "", "", $group_by);
    // print_r($asset_type_list);

    if ($asset_type_list->status) {
        return $asset_type_list->data;
    } else {
        print_r($asset_type_list);
        return 0;
    }
...
function type_of_equipment($unique_id = "", $digital_category = "")
{
    // echo $zone_name;
    global $pdo;
    ...
    if ($digital_category) {
        $where = [];
        $where["digital_category"] = $digital_category;
    }
    // print_r($table_details);


    $digital_asset_options = $pdo->select($table_details, $where);
    // print_r($facility_name_options);

    if ($digital_asset_options->status) {
        return $digital_asset_options->data;
    } else {
        print_r($digital_asset_options);
        return 0;
    }
}

function btn_edits($folder_name = "", $unique_id = "", $tab_type = "")
{
    $password = '3sc3RLrpd17';
    $enc_method = 'aes-256-cbc';
    $enc_password = substr(hash('sha256', $password, true), 0, 32);
    ...
    // Add the `tab_type` parameter (either 'kitchen' or 'digital')
    $final_str = '<a href="index.php?file=' . $file_name_update . '&unique_id=' . $uni_id . '&tab_type=' . $tab_type . '" class="font-18 text-info me-2" data-bs-toggle="tooltip" data-bs-placement="top" aria-label="Edit" data-bs-original-title="Edit"><i class="uil uil-pen"></i></a>';
...
function transfer_reject_reason($unique_id = '')
{
    global $pdo;

    $table_name = 'transfer_reject_reason';
    $where = [];
    $table_columns = [
...
function transfer_std_list()
{
    global $pdo;

    $table_name = 'student_transfer';

    $table_columns = [
        "GROUP_CONCAT(CONCAT('\"', std_id, '\"') SEPARATOR ', ') AS std_id"
    ];
    ...
    $where = "approval_status != '2' and is_delete = '0' and from_hostel = '" . $_SESSION['hostel_id'] . "'";

    $district_name_list = $pdo->select($table_details, $where);
    // print_r($district_name_list);
    if ($district_name_list->status) {
        return $district_name_list->data[0]['std_id'];
    } else {
        print_r($district_name_list);
...
function exit_reason($unique_id = "")
{
    global $pdo;

    $table_name = "exit_reason";
    $where = [];
    $table_columns = [
        "exit_reason AS unique_id",
        "exit_reason"
    ];
    ...
    $exit_reason_list = $pdo->select($table_details, $where);
    // print_r($exit_reason_list);
    if ($exit_reason_list->status) {
        return $exit_reason_list->data;
    } else {
        print_r($exit_reason_list);
        return 0;
    }
...
function student_reg_list($unique_id = "", $hostel_id = "", $student_list = "", $student_type = "")
{
    ...
    $where = "is_delete = '0' and dropout_status != 2 and academic_year = '6847e16e7060491061'";
    ...
    if ($student_list) {
        $where = [];
        $where = "";
        $where .= 'unique_id NOT IN (' . $student_list . ') and is_delete = "0" and dropout_status != 2 and hostel_1 = "' . $hostel_id . '" and academic_year = "6847e16e7060491061"';
        // $where = "";
    }
...
function checkbox_reason($unique_id = "")
{
    global $pdo;

    $table_name = "checkbox_reason";
    $where = [];
    $table_columns = [
        "reason AS unique_id",
        "reason"
...
function item_category_stock_inward($unique_id = "")
{
    global $pdo;
    ...
    $where = "is_delete = 0 AND unique_id != 'ec8c229e224e3c7879'";
...
function item_categoryindent($unique_id = "")
{
    global $pdo;

    $table_name = "monthly_indent_items";
    $table_columns = [
        "category as unique_id",
        "(SELECT item_category FROM item_category WHERE is_delete=0 AND unique_id=monthly_indent_items.category) as category"
    ];
...
function item_stock_monthinward($unique_id = "", $category = "", $hostel_id = "")
{
    global $pdo;
    ...
    $table_columns = [
        "item as unique_id",
        "(SELECT item FROM item WHERE is_delete=0 AND unique_id=monthly_indent_items.item) as item"
    ];
    ...
    // Default where condition
    $where = "is_delete = 0 AND is_active = 1 AND hostel_id = '$hostel_id'";
...
function item_type_name($unique_id = "", $product_category = "", $ex_stk_val = "")
{
    global $pdo;
    ...
    if ($product_category) {
        $where .= " AND category_id = '$product_category'";
    }

    if ($ex_stk_val) {
        $where .= " AND unique_id NOT IN ($ex_stk_val)";
    }

    $item_type_name = $pdo->select($table_details, $where);

    if ($item_type_name->status) {
