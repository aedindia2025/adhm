...
$where = 'new_uuid IS NULL';
            ...
            foreach ($uuids as $device) {
                $uuid = $device["temp_aadhar"];

                // API endpoint
                // $url = 'https://tnpreauth.tn.gov.in/auakua25dvuat/api/datavault/GetUID?UUID=' . $uid . '&AppKey=MQOOT-BXPBZ-COZRJ-UGHZQ&UIDType=1';
$url = 'https://tnauth.tn.gov.in/auakua25dv/api/datavault/GetUUID?UID='. $uuid .'&AppKey=IDKJS-PPSSJ-VKAZE-DMCCG&UIDType=3';
                // Initialize cURL
                $ch = curl_init($url);
                ...
                if (isset($response_data['RESPONSE']['UUID'])) {
                    $uid = $response_data['RESPONSE']['UUID'];

                    // Create a MySQLi connection
                    // $mysqli = new mysqli("localhost", "root", "H_Cw3O4CM*fXcGtz", "adi_dravidar");
                    ...
                    $stmt = $mysqli->prepare("UPDATE aadhar SET new_uuid = ? WHERE temp_aadhar = ?");
                    if (!$stmt) {
